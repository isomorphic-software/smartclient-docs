...
### Description
A lightweight [CoTProcess](CoTProcess.md#class-cotprocess) that routes user requests to the most appropriate registered [AI service](../reference.md#object-aiservicedescriptor). It uses a single CoT step to ask the AI which service best matches the user's intent, then invokes that service with the original prompt and the AI's rationale.
