...
### Description
A widget for editing a single, advanced [hilite rule](HiliteRule.md#class-hiliterule) for use by [dataBoundComponents](../main.md#interface-databoundcomponent). Where a simple hilite provides configuration of a single criterion and either foreground or background color for application to a single field, an advanced hilite can specify more complex criteria which can both test and affect multiple fields and allow both background and foreground colors to be specified in a single rule.
...
This component is an [AutoChild](../main.md#type-autochild) and as such may be customized via `advancedHiliteEditor.filterBuilderProperties`.
...
This component is an [AutoChild](../main.md#type-autochild) and as such may be customized via `advancedHiliteEditor.hiliteFormProperties`.
...
This component is an [AutoChild](../main.md#type-autochild) and as such may be customized via `advancedHiliteEditor.cancelButtonProperties`.
...
`hiliteIcons` should be specified as an Array of [SCImgURL](../main.md#type-scimgurl). When present, [hilite rules](HiliteRule.md#class-hiliterule) will offer the user a drop down for picking one of these icons.
...
This component is an [AutoChild](../main.md#type-autochild) and as such may be customized via `advancedHiliteEditor.saveButtonProperties`.
