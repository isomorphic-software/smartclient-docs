...
`valueQuery` is supported for client-side filtering of `clientOnly` DataSources, [MockDataSource](MockDataSource.md#class-mockdatasource)s, and DataSources with a complete cache. See the "Client-Side Subquery Support" section in the [subquery overview](../reference.md#object-advancedcriterionsubquery) for details.
...
### Description
A constant value to use as the left-hand side of the comparison, instead of a field value from the record. When set, this value is used in place of the field value that would normally be derived from [Criterion.fieldName](#attr-criterionfieldname) or [Criterion.fieldQuery](#attr-criterionfieldquery).

This is useful when comparing a constant value against the result of a [valueQuery](#attr-criterionvaluequery). For example, to check whether a particular product code appears in the set of products ordered by a customer:
