...
Note, `dsRequestModifier` values are evaluated during [DSRequest](../reference_2.md#object-dsrequest) setup, before the request's `execute()` method is called. This means that variables added to the Velocity context by calling `addToTemplateContext()` from a [DMI](../kb_topics/dmiOverview.md#kb-topic-direct-method-invocation) method or [custom DataSource implementation](DataSource_1.md#attr-datasourceserverconstructor) will not be available. In this case, you can either
