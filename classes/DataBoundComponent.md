...
NOTE: if `autoFetchData` is set, calling [fetchData()](ListGrid_1.md#method-listgridfetchdata) before draw will cause two requests to be issued, one from the manual call to fetchData() and one from the autoFetchData setting. The second request will use only [DataBoundComponent.initialCriteria](#attr-databoundcomponentinitialcriteria) and not any other criteria or settings from the first request. Generally, turn off autoFetchData if you are going to manually call [fetchData()](ListGrid_1.md#method-listgridfetchdata) at any time.
...
User-added formula fields can be persisted via [ListGrid.getFieldState](ListGrid_1.md#method-listgridgetfieldstate) and [ListGrid.setFieldState](ListGrid_2.md#method-listgridsetfieldstate).
...
### Description
If this attribute is set to false, any attributes in the component's values object that do not map to a [field](../reference.md#object-datasourcefield) or [formItem](FormItem.md#class-formitem) will not be tracked when checking for changes. You should only set this flag to false if you know that your code does not store additional, non-field values in the component's data, or if you do store such values, but you don't care that they are not checked for changes. This flag is primarily provided to avoid performance issues in cases where developers are storing large numbers of extra attributes in component data; generally speaking, you should only consider setting it to false if you have a use case like this.
...
User-added summary fields can be persisted via [ListGrid.getFieldState](ListGrid_1.md#method-listgridgetfieldstate) and [ListGrid.setFieldState](ListGrid_2.md#method-listgridsetfieldstate).
...
### Description
Does this component represent singular or multiple "records" objects? Options are "multiple" or "single", or "either"
...
---
## Attr: DataBoundComponent.canReorderFields

### Description
Can this component's fields be reordered. For some components (such as [ListGrid](ListGrid_1.md#class-listgrid)) this flag enables a user-interface for reordering fields in realtime. For components where this is not the case, setting this flag to true may have no visible impact in normal usage but may be consulted by the framework when integrating with a [FieldPicker](FieldPicker.md#class-fieldpicker)
...
| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| field | [Field](#type-field) | false | — | Field to edit or null to add a new formula field |
...
`[Object](../reference_2.md#type-object)` — Field description
...
| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| criteria | [Criteria](../reference.md#type-criteria) | false | — | non user-editable criteria to apply to this component |
| callback | [DSCallback](../reference.md#type-dscallback) | true | — | callback to fire on successful fetch - will not fire if `willFetchData` is false or if an initial fetch is required but `` initialFetch `was not passed` `` |
...
| return | [FieldState](../reference.md#type-fieldstate) | false | — | Current field state |
...
### Description
For a component with a specified [DataSource](DataSource.md#class-datasource), find the associated dataSource field object from a specified [dataPath](../reference.md#type-datapath).

### Parameters

| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| dataPath | [DataPath](../reference.md#type-datapath) | false | — | dataPath for which the field definition should be returned. |
