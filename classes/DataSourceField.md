...
- [SequenceMode](../main.md#type-sequencemode)
...
### Description
Indicates this field must be non-null in order for a record to pass validation. Or, in the case of a "binary" field, a non-empty file must be uploaded. Setting this property has the same effect as giving the field a ["required" validator](../main.md#type-validatortype).
...
This will be picked up by [DataBoundComponent](../main.md#interface-databoundcomponent)s and other views over this data source.
...
### Description
For fields that are [multiple:true](#attr-datasourcefieldmultiple) and use [DataSourceField.multipleStorage](#attr-datasourcefieldmultiplestorage), the separator used in the "simpleString" [MultipleFieldStorage](../main.md#type-multiplefieldstorage) mode.
...
### Description
Width of the image-content of this field. If set as a string, represents the name of another field in the record that holds the imageWidth. Applicable only to fields of image type or fields that use a [ViewFileItem](../main.md#class-viewfileitem) as an editor.
...
### Description
This property is only applicable to fields of SQL DataSources, it overrides default behavior of [iContains](../main.md#type-operatorid) case insensitive operator on the server-side. Set to `true` to force case insensitivity by changing case of both comparison expression sides or `false` to rely on database `LIKE` operator directly.  
...
### Description
Indicates that the specified [VelocityExpression](../main_2.md#type-velocityexpression) must evaluate to true if initializations of this field are to be allowed. If the specified expression does not evaluate to true, the field will be removed from the request as described for [editRequiresAuthentication](#attr-datasourcefieldeditrequiresauthentication).
...
*   Components cannot bind to the field; even if you explicitly add a field with the same name to your [dataBoundComponent](../main.md#interface-databoundcomponent), it will be dropped
...
*   If the field is editable, it will be marked [hidden](#attr-datasourcefieldhidden). By default, this means that [DataBoundComponent](../main.md#interface-databoundcomponent)s will not show the field, though this can be overridden for individual components. If you do choose to unhide the field, bear in mind that it will never actually show any data, because the underlying security test has still failed
...
`foreignKey` declarations also allow other automatic behaviors by [DataBoundComponents](../main.md#interface-databoundcomponent), such as [ListGrid.fetchRelatedData](ListGrid_2.md#method-listgridfetchrelateddata).
...
### Description
If `field.template` is set, this field's value in records will be calculated dynamically via the specified [template](../main.md#object-usersummary).
...
When records are displayed in [dataBoundComponents that support editing](../main.md#interface-databoundcomponent), template field values will be re-calculated dynamically on the client as the user edits a record, so a user may preview the result of their changes.
...
`DataSourceField.template` is supported for [SQL DataSources](../main_2.md#type-dsservertype), and for [clientOnly dataSources](DataSource.md#attr-datasourceclientonly) only.
...
### Description
Indicates that the specified [VelocityExpression](../main_2.md#type-velocityexpression) must evaluate to true if updates to this field are to be allowed. If the specified expression does not evaluate to true, the field will be removed from the request as described for [editRequiresAuthentication](#attr-datasourcefieldeditrequiresauthentication).
...
If unset, a FormItem will be automatically chosen based on the type of the field, by the rules explained [here](../main.md#type-formitemtype).
...
*   **dsRequest** The current [DSRequest](../main_2.md#object-dsrequest)
...
### Description
If [ListGrid.showGridSummary](ListGrid_1.md#attr-listgridshowgridsummary) or [ListGrid.showGroupSummary](ListGrid_1.md#attr-listgridshowgroupsummary) is true, this attribute can be used to specify an explicit [SummaryFunction](../main_2.md#type-summaryfunction) for calculating the summary value to display.
...
### Description
Maximum number of characters allowed. Applicable only to fields of text type. For fields of this type a [length range validator](../main.md#type-validatortype) will be automatically generated on both the client and server side to enforce this maximum length (unless such a validator is explicitly present for the field already).
...
### Description
Fully qualified Java className of an Enum that should be used to derive this field's [ValueMap](../main_2.md#type-valuemap).
...
### Description
For fields of a type that are represented by binary data initially uploaded from a file (currently "binary" and "imageFile", see [FieldType](../main_2.md#type-fieldtype)), this sets the maximum file size allowed, in bytes.
...
### Description
A [Callback](../main.md#type-callback), function, or JavaScript expression used to retrieve the field's value from the XML element or JSON record returned from a web service.
...
Must be unique across all fields within the DataSource as well as a valid JavaScript identifier - see [FieldName](../main.md#type-fieldname) for details and how to check for validity.
...
*   AdvancedCriteria [operators](../main.md#type-operatorid) that check if values are in a particular range will always pass
...
Use this flag to inhibit the normal use of [TextMatchStyle](../main_2.md#type-textmatchstyle) for this field. A field with this flag set will always be tested for exact equality in generated queries, even for filter-style queries where normal behavior would be to use a substring match or similar.
...
### Description
When [field.includeFrom](#attr-datasourcefieldincludefrom) is specified and multiple records exist in the related DataSource per record in the including DataSource, `includeSummaryFunction` indicates which [SummaryFunction](../main_2.md#type-summaryfunction) is used to produce the field value.
...
*   using "concat" to show the names of all "salesReps" involved in an "order" (note: "concat" has limited support - see [SummaryFunction](../main_2.md#type-summaryfunction)).

**NOTE**: `includeSummaryFunction` and [Server Summaries](../kb_topics/serverSummaries.md#kb-topic-server-summaries) cannot be used in the same [DSRequest](../main_2.md#object-dsrequest). If both configurations are present, Server Summaries settings always take priority.
...
For simple criteria, note that `$criteriaOperator` will vary depending on field type and the [textMatchStyle](../main_2.md#type-textmatchstyle) in force, as follows:
...
### Description
Height of the image-content of this field. If set as a string, represents the name of another field in the record that holds the imageHeight. Applicable only to fields of image type or fields that use a [ViewFileItem](../main.md#class-viewfileitem) as an editor.
...
### Description
Defines prefix before concatenated values if field is used with [Server summaries](../kb_topics/serverSummaries.md#kb-topic-server-summaries) feature and the [summary function](../main_2.md#type-summaryfunction) is "concat".
...
### Description
Defines suffix after concatenated values if field is used with [Server summaries](../kb_topics/serverSummaries.md#kb-topic-server-summaries) feature and the [summary function](../main_2.md#type-summaryfunction) is "concat".
...
The default `multipleStorage` setting of null means that the "simpleString" [MultipleFieldStorage](../main.md#type-multiplefieldstorage) approach will be used if:
...
Otherwise, [MultipleFieldStorage](../main.md#type-multiplefieldstorage) "none" will be used.
...
*   "between" and all other operators with [OperatorValueType](../main_2.md#type-operatorvaluetype) of "valueRange"
...
### Description
Defines the delimiter between concatenated values if field is used with [Server summaries](../kb_topics/serverSummaries.md#kb-topic-server-summaries) feature and the [summary function](../main_2.md#type-summaryfunction) is "concat". The default value is ", ".
...
### Description
Title to show in a [Summary of type "title"](../main_2.md#type-summaryfunction) for this field. If unspecified `title` summaries will show the [DataSourceField.title](#attr-datasourcefieldtitle) for the field.
...
`DataSourceField.formula` is supported for [SQL DataSources](../main_2.md#type-dsservertype) and for [clientOnly dataSources](DataSource.md#attr-datasourceclientonly) only.
...
When records are displayed in [dataBoundComponents that support editing](../main.md#interface-databoundcomponent), formula field values will be re-calculated dynamically on the client as the user edits a record, so a user may preview the result of their changes.
...
### Description
Indicates that the specified [VelocityExpression](../main_2.md#type-velocityexpression) must evaluate to true if writes to this field are to be allowed. If the specified expression does not evaluate to true, the field will be removed from the request as described for [editRequiresAuthentication](#attr-datasourcefieldeditrequiresauthentication).
...
Only applicable to fields of a type that are represented by binary data (currently "binary" and "imageFile", see [FieldType](../main_2.md#type-fieldtype)).
...
### Description
Width and height of the image-content of this field. If set as a string, represents the name of another field in the record that holds the imageSize. Applicable only to fields of image type or fields that use a [ViewFileItem](../main.md#class-viewfileitem) as an editor.
...
For [AdvancedCriteria](../main.md#object-advancedcriteria), for normal [search operators](../main.md#type-operatorid) the field value is considered as matching the `Criterion` if _any_ of the field values match the Criterion. Specifically, this is true of all operators that have an [operatorValueType](../main_2.md#type-operatorvaluetype) of "fieldType" or "valueRange".
...
### Description
Indicates that the specified [VelocityExpression](../main_2.md#type-velocityexpression) must evaluate to true if values for the field are to be fetched. If the specified expression does not evaluate to true, the field will be dropped as described for [viewRequiresAuthentication](#attr-datasourcefieldviewrequiresauthentication).
...
### Description
Whether this field should be considered a "detail" field by a [DataBoundComponent](../main.md#interface-databoundcomponent).
...
For example, the [VLayout](../main.md#class-vlayout) schema sets this for its [members](Layout.md#attr-layoutmembers) property, so that when a VLayout is constructed via XML as follows:
...
This is a per-field setting; you can alternatively set a default format for all "date", "time" or "datetime" fields via [DateUtil.setNormalDatetimeDisplayFormat](DateUtil.md#classmethod-dateutilsetnormaldatetimedisplayformat) and related methods on [Date](../main_2.md#object-date). See also [localizedNumberFormatting](../main.md#kb-topic-localized-number-formatting) for built-in [FieldTypes](../main_2.md#type-fieldtype) that handle localized currency formatting.
