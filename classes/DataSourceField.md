...
### Description
For fields that are [multiple:true](#attr-datasourcefieldmultiple) and use [DataSourceField.multipleStorage](#attr-datasourcefieldmultiplestorage), the separator used in the "simpleString" [MultipleFieldStorage](../reference_2.md#type-multiplefieldstorage) mode.
...
### Description
When using [DataSource.recordsAsText](DataSource_2.md#method-datasourcerecordsastext), determines what approach (if any) should be used to force values to be intepreted as text instead of heuristically parsed as dates, times or other structured types.
...
```
 <field name="priority">
     <updateWhen fieldName="status" operator="notEqual"
                 value="Closed"/>
 </field>
...
### Description
For a DataSource with [serverType](DataSource_1.md#attr-datasourceservertype) "sql", "hibernate", or "jpa", indicates that this field should be omitted by default from all SQL operations, and will only be used with [custom queries](../kb_topics/customQuerying.md#kb-topic-custom-querying-overview).
...
For SQL DataSources, values are calculated on the server by modifying the generated SQL request as appropriate. As with other [dynamically calculated](DataSource_2.md#method-datasourceiscalculated) fields, fields with a specified template are non editable.  
...
```
 <field name="category">
     <initWhen fieldName="auth.roles" operator="contains"
               value="admin"/>
 </field>
...
---
## Attr: DataSourceField.customJoinExpression

### Description
For fields with [DataSourceField.includeFrom](#attr-datasourcefieldincludefrom), allows customization of the SQL JOIN condition used to retrieve this field's value from the related DataSource.

The value is a Velocity expression used as the ON clause of the generated JOIN. The variable `$defaultJoinExpression` (alias `$defaultJoin`) contains the auto-generated join condition (the equality test on foreign key columns), allowing you to extend the default condition rather than replace it entirely.

For example, to add an additional constraint to the join:

```
   <field name="capital" includeFrom="cityDS.name"
          customJoinExpression="$defaultJoinExpression
                                AND cityDS.isActive = 1" />
 
```
This generates SQL like:
```
   JOIN cityDS ON countryDS.capitalId = cityDS.id AND cityDS.isActive = 1
 
```
instead of:
```
   JOIN cityDS ON countryDS.capitalId = cityDS.id
 
```

The [standard Velocity context variables](../kb_topics/velocitySupport.md#kb-topic-velocity-context-variables) are available. Unlike [DataSourceField.customSelectExpression](#attr-datasourcefieldcustomselectexpression), values in `customJoinExpression` are **not** automatically quoted, because this expression represents a SQL condition rather than a value.

In addition, the following variables are available for building or extending the join condition:

*   `$defaultJoinExpression` (shortcut `$defaultJoin`) — the auto-generated join condition (equality test on foreign key columns). This is typically used as a base and extended with additional constraints, as shown in the example above.
*   `$fromTable` (shortcut `$from`) — table name or alias of the DataSource that declares the foreign key (the "from" side)
*   `$toTable` (shortcut `$to`) — table name or alias of the related DataSource (the "to" side)

These support Velocity dot-syntax for qualified column references: `$from.columnName` resolves to `tableName.columnName` (or `alias.columnName` when [DataSourceField.relatedTableAlias](#attr-datasourcefieldrelatedtablealias) is set).

Using `$from/$to` instead of hardcoded table names is strongly recommended. When a field with `customJoinExpression` is reached via chained [DataSourceField.includeFrom](#attr-datasourcefieldincludefrom) (for example, DS\_A includes from DS\_B, which includes from DS\_C), the actual table names or aliases depend on the query context and may differ from those declared in the DataSource. Hardcoded names can produce incorrect SQL in such cases, while `$from/$to` always resolve to the correct table or alias. As a secondary benefit, the expression becomes resilient to table renames and compatible with [DataSourceField.relatedTableAlias](#attr-datasourcefieldrelatedtablealias).

```
   <field name="capital" includeFrom="cityDS.name"
          customJoinExpression="$from.capitalId = $to.id
                                AND $to.isActive = 1" />
 
```

Applies to SQL DataSources only. Works with both ANSI joins ([useAnsiJoins:true](DataSource_1.md#attr-datasourceuseansijoins)) and non-ANSI WHERE-clause joins.
...
If `stringInBrowser` is not set, the default behavior is configured by [DataSource.stringInBrowser](DataSource_1.md#attr-datasourcestringinbrowser) on the owning DataSource if set, otherwise by the `server.properties` setting `datasource.defaultStringInBrowser`. If this flag is false, numeric values are delivered to the client as numbers, _even where this will lead to a loss of precision_. If the flag is true (which is the default), the behavior is to prevent range overflow for numeric values:
...
Also note that the above example does not provide a DataSource-level [unionFields](DataSource_1.md#attr-datasourceunionfields) setting. This means that we will derive a list of fields to union by inspecting every member dataSource and unioning fields with matching names and data types (see [defaultUnionFieldsStrategy](DataSource_1.md#attr-datasourcedefaultunionfieldsstrategy) for more details of how this is done). However, fields `ds1.someField` and `ds2.someOtherField` will not be part of the auto-derivation, because the system will see that they are being explicitly referenced in a field's `unionOf` property.
...
If explicitly set, this property will be respected by [DataSource.isCalculated](DataSource_2.md#method-datasourceiscalculated)
...
The default `multipleStorage` setting of null means that the "simpleString" [MultipleFieldStorage](../reference_2.md#type-multiplefieldstorage) approach will be used if:
...
Otherwise, [MultipleFieldStorage](../reference_2.md#type-multiplefieldstorage) "none" will be used.
...
```
 <field name="quantity">
     <editWhen fieldName="status" operator="equals"
               value="Pending"/>
 </field>
...
---
## Attr: DataSourceField.expensive

### Description
Explicitly marks a field as expensive or inexpensive to fetch, overriding the default heuristics used by the server-side `DataSource.isExpensive()` method.

When [DSRequest.addInexpensiveFields](DSRequest.md#attr-dsrequestaddinexpensivefields) is active, the server automatically expands the requested [DSRequest.outputs](DSRequest.md#attr-dsrequestoutputs) to include all fields not considered expensive. By default, the following types of fields are considered expensive:

*   [Binary](../reference_2.md#type-fieldtype) fields with [DataSourceField.encodeInResponse](#attr-datasourcefieldencodeinresponse) set
*   Text fields with a declared [DataSourceField.length](#attr-datasourcefieldlength) exceeding the server threshold (default 255; configurable via `sql.expensiveFieldLengthThreshold` in `server.properties`)
*   Fields with a [DataSourceField.outputWhen](#attr-datasourcefieldoutputwhen) setting
*   Fields marked [detail:true](#attr-datasourcefielddetail)
*   [DataSourceField.includeFrom](#attr-datasourcefieldincludefrom) fields whose SQL join is not already required by other requested fields

Setting `expensive:true` forces a field to be treated as expensive even if no default heuristic applies — for example, a field with a costly [DataSourceField.customSelectExpression](#attr-datasourcefieldcustomselectexpression) or one that triggers an expensive database operation.

Setting `expensive:false` forces a field to be treated as inexpensive even if a heuristic would otherwise flag it — for example, an [DataSourceField.includeFrom](#attr-datasourcefieldincludefrom) field that is always needed regardless of whether its join is required by other fields.

When this property is unset (the default), the server's `isExpensive()` method applies its default heuristics.

### See Also

- [DSRequest.addInexpensiveFields](DSRequest.md#attr-dsrequestaddinexpensivefields)
...
As with other [dynamically calculated](DataSource_2.md#method-datasourceiscalculated) fields, fields with a specified formula are non editable.  
...
Note that fields with an `outputWhen` setting are also considered [expensive](#attr-datasourcefieldexpensive) by default when [DSRequest.addInexpensiveFields](DSRequest.md#attr-dsrequestaddinexpensivefields) is active, since the presence of conditional output logic implies the field has a cost concern.
...
- [DataSourceField.expensive](#attr-datasourcefieldexpensive)
...
The table name will also be used to qualifying the column name unless [DataSource.qualifyColumnNames](DataSource_1.md#attr-datasourcequalifycolumnnames) or [OperationBinding.qualifyColumnNames](OperationBinding.md#attr-operationbindingqualifycolumnnames) has been disabled.

**Note:** This property is only required when including fields from tables other than the default table specified by [DataSource.tableName](DataSource_1.md#attr-datasourcetablename).
