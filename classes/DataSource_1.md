...
---
## Attr: DataSource.stringInBrowser

### Description
Server-side default for [DataSourceField.stringInBrowser](DataSourceField.md#attr-datasourcefieldstringinbrowser) for every field of this DataSource that does not set `stringInBrowser` explicitly. Provides per-DataSource control equivalent to the `server.properties` setting `datasource.defaultStringInBrowser`, but scoped to this one DataSource.

The default that governs the precision-based stringInBrowser behavior resolves most-specific-first: an explicit field-level [DataSourceField.stringInBrowser](DataSourceField.md#attr-datasourcefieldstringinbrowser) (a hard per-field override) wins outright; otherwise this DataSource-level setting applies; otherwise the global `datasource.defaultStringInBrowser` property; otherwise the built-in default of `true`.

Set this to `false` on a DataSource whose high-precision numeric columns are formatted or rounded for display, so their aggregate (for example [summary](ListGrid_1.md#attr-listgridshowgridsummary)) `SUM` values are delivered as numbers and honor [field formatters](DataSourceField.md#attr-datasourcefieldformat), rather than being stringified to preserve precision the application is not using. This affects only the individual fields that do not set `stringInBrowser`; it does not weaken precision preservation for any other DataSource.

Unlike the [DataSource.defaultStringInBrowser](DataSource.md#classattr-datasourcedefaultstringinbrowser) class default, this attribute applies when using the SmartClient Server.
...
- [DataSource.transformResponseScript](#attr-datasourcetransformresponsescript)
- [OperationBinding.transformRawResponseScript](OperationBinding.md#attr-operationbindingtransformrawresponsescript)
...
---
## Attr: DataSource.velocityInCriteriaEnabled

### Description
DataSource-level default for [OperationBinding.velocityInCriteriaEnabled](OperationBinding.md#attr-operationbindingvelocityincriteriaenabled). Applies to all operationBindings on this DataSource that do not set the flag explicitly.
...
### Description
Controls whether [AdvancedCriteria subqueries](../reference.md#object-advancedcriterionsubquery) are permitted for this `DataSource`. Set this value `true` to always allow subqueries in criteria on this dataSource, and set it false to always disallow them.
...
On the client, this property is also applied to `clientOnly` DataSources, [MockDataSource](MockDataSource.md#class-mockdatasource)s, and DataSources with a complete cache before processing [fieldQuery](Criterion.md#attr-criterionfieldquery) or [valueQuery](Criterion.md#attr-criterionvaluequery) criteria. When set to `false`, subquery criteria are ignored during client-side filtering.
...
| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| model | [Object](../reference_2.md#type-object) | false | — | the export model, of the form:
```
   {
...
| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| fieldNames | [Array of String](#type-array-of-string) | false | — | Valid field names for `fieldName` enum |
| allOperatorIds | [Array of String](#type-array-of-string) | false | — | All valid operator IDs (logical + field) |
| fieldOperatorIds | [Array of String](#type-array-of-string) | false | — | Field- level operator IDs (excludes and/or/not) |
| dsID | [String](#type-string) | true | — | DataSource ID for naming the generated schema |

### Returns

`[DataSource](#type-datasource)` — Criteria schema DataSource
...
`[Object](../reference_2.md#type-object)` — Map of dataSourceId -> Array of DSRequest
...
---
## Method: DataSource.loadInReify

### Description
Open this DataSource in Reify for editing. The DataSource must be stored in Reify's DataSource storage; if not, a warning is logged and the call fails.
...
---
## Method: DataSource.asJSONSchema

### Description
Returns a JSON Schema (Draft 2020-12) representation of this DataSource. The schema describes the structure, types, constraints, and optionally events of records conforming to this DataSource.

DataSources referenced by fields (complex types) are serialized as `$defs` entries with `$ref` pointers (or `definitions` / `$ref` when [JSONSchemaSettings.useDefs](JSONSchemaSettings.md#attr-jsonschemasettingsusedefs) is false).

Inheritance via [DataSource.inheritsFrom](#attr-datasourceinheritsfrom) is represented using `allOf` referencing the parent schema definition plus the local properties. Fields merged from ancestors appear in the flattened properties, but the `allOf` structure preserves the inheritance relationship.

Fields marked [inapplicable:true](DataSourceField.md#attr-datasourcefieldinapplicable) are output with the hardcoded `false` schema, meaning any value supplied for that property will fail validation. This is the standard JSON Schema idiom for "this property must not be used."

The SmartClient `"measure"` type accepts either a plain integer (pixels) or a string like `"50%"` or `"*"`. Because SmartClient has no native union-type concept, this is implemented with hard-coded validation, but `asJSONSchema()` translates it to a `oneOf` schema: `[{type:"integer"}, {type:"string", pattern:...}]`.

See [JSONSchemaSettings](../reference_2.md#object-jsonschemasettings) for available options.

### Parameters

| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| settings | [JSONSchemaSettings](#type-jsonschemasettings) | true | — | Optional settings controlling the output format and content. |

### Returns

`[Object](../reference_2.md#type-object)|[String](#type-string)` — JSON Schema object, or a JSON string if [JSONSchemaSettings.asString](JSONSchemaSettings.md#attr-jsonschemasettingsasstring) is true.
...
---
## Method: DataSource.getEventSchema

### Description
Returns a DataSource describing the event/action binding for a named event on this DataSource's corresponding SmartClient class.

An event/action binding always has at least two fields: `target` (the ID of the component to invoke the action on) and `name` (the method to call on the target). The actual invocation is roughly `target[name](...mapped params)`.

When `settings.autoBindEvents` is false, a `mapping` field is included: an ordered array of the source event's parameter names, in the order they should be passed to the target method. When auto-binding is on (the default when JSDoc data is loaded), parameters are wired automatically and `mapping` is omitted.

The `mode` parameter selects between:

*   `"actionBinding"` (default) – target + name + optional mapping
*   `"declarative"` – adds Process definition fields for full workflow support

### Parameters

| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| eventName | [String](#type-string) | false | — | Name of the event (e.g. "click", "recordClick") |
| settings | [JSONSchemaSettings](#type-jsonschemasettings) | true | — | Controls autoBindEvents and mode. If a String is passed, it is treated as the mode for convenience. |
...
### Description
Get the list of [Operator](../reference_2.md#object-operator)s available for this field, as a [ValueMap](../reference_2.md#type-valuemap) from [OperatorId](../reference.md#type-operatorid) to the [Operator.title](Operator.md#attr-operatortitle) specified for the [Operator](../reference_2.md#object-operator), or the corresponding property in [Operators](Operators.md#class-operators) if [Operator.titleProperty](Operator.md#attr-operatortitleproperty) is set.
...
`[Object](../reference_2.md#type-object)` — Object with properties: position (int), relative (String), ambiguous (boolean)
...
### Description
Returns a JSON Schema for a DSRequest `outputs` value against this DataSource. The emitted schema is the array-of-field-names form, which is what allows the names to be constrained to an enum; `outputs` itself also accepts the documented comma-separated string.
...
---
## Method: DataSource.getCriteriaSchema

### Description
Returns a DataSource that describes valid [AdvancedCriteria](../reference.md#object-advancedcriteria) for this DataSource, with `fieldName` constrained to an enum of this DataSource's field names, `operator` constrained to valid operators for those fields, and `valuePath` constrained to valid field data paths.

The returned DataSource can be passed to [DataSource.asJSONSchema](#method-datasourceasjsonschema) to produce JSON Schema suitable for guiding AI-based criteria construction.

The schema separates two kinds of criteria via a `oneOf` pattern (expressed as two internal DataSources that become `$defs` entries when serialized to JSON Schema):

*   **FieldCriterion**: has `fieldName`, `operator`, `value`/`valuePath`, `start`/`end`
*   **LogicalCriterion**: has `operator` (and/or/not) and `criteria` (array of nested criteria)

This separation matches the server-side Java class hierarchy and produces clearer JSON Schema for AI consumers.

Caching: the result is cached when `settings` is not passed or is an empty object. Subsequent calls with settings bypass the cache.

### Parameters

| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| settings | [JSONSchemaSettings](#type-jsonschemasettings) | true | — | Optional settings (currently reserved for future use) |

### Returns

`[DataSource](#type-datasource)` — DataSource describing valid AdvancedCriteria for this DataSource
...
---
## Method: DataSource.isInReify

### Description
Check whether this DataSource is stored in Reify's DataSource storage.
