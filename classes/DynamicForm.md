...
### Description
When true, all items resolve to the Inline\* variant of their standard editor type when such a class exists (e.g. TextItem → InlineTextItem). Inline items render with rounded borders and density-scaled metrics suited to TW-derived skins.
...
---
## Attr: DynamicForm.largeValueMapFilterEditorThreshold

### Description
For [SearchForms](SearchForm.md#class-searchform) and [Items in a filterEditor](ListGrid_1.md#attr-listgridshowfiltereditor) this property governs the threshold for how many entries are in a [FormItem.valueMap](FormItem.md#attr-formitemvaluemap) before the [DynamicForm.largeValueMapFilterEditorType](#attr-dynamicformlargevaluemapfiltereditortype) is displayed.

Only applies to fields with an explicit valueMap - if a field derives its options from an [optionDataSource](FormItem.md#attr-formitemoptiondatasource), the largeValueMapFilterEditorType is always used.
...
### Description
The string pre-pended to the title of every item in this form. See also [DynamicForm.requiredTitlePrefix](#attr-dynamicformrequiredtitleprefix) for fields that are required.
...
### Description
The string appended to the title of every item in this form. See also [DynamicForm.requiredTitleSuffix](#attr-dynamicformrequiredtitlesuffix) for fields that are required.
...
---
## Attr: DynamicForm.linearHintWidth

### Description
When [DynamicForm.linearMode](#attr-dynamicformlinearmode) is true, this property sets the amount of space reserved for [hint-text](FormItem.md#attr-formitemhint) in formItems that set a hint. May be set to a pixel-size like 100 or a percentage, such as "30%".
...
---
## Attr: DynamicForm.useFileUploadItem

### Description
If true, use [FileUploadItem](FileUploadItem.md#class-fileuploaditem) as the default editor for `binary` type fields instead of the standard [UploadItem](UploadItem.md#class-uploaditem).

This enables HTML5 drag-and-drop file upload with progress indication.

This property can also be set on individual fields via [FormItem.editorType](FormItem.md#attr-formitemeditortype).
...
Also note that enabling `allowExpressions` for an entire form changes the [DynamicForm.defaultSearchOperator](#attr-dynamicformdefaultsearchoperator) to ["iContainsPattern"](DataSource_1.md#attr-datasourcetranslatepatternoperators), so that simple search expressions similar to SQL "LIKE" patterns can be entered in most fields.
...
### Description
Default [search operator](../reference.md#type-operatorid) to use for fields in a form that produces [AdvancedCriteria](../reference.md#object-advancedcriteria). Default is "iContains" unless [DynamicForm.allowExpressions](#attr-dynamicformallowexpressions) is enabled for the form as a whole, in which case the default is ["iContainsPattern"](DataSource_1.md#attr-datasourcetranslatepatternoperators).
...
---
## Attr: DynamicForm.largeValueMapFilterEditorType

### Description
For [SearchForms](SearchForm.md#class-searchform) and [Items in a filterEditor](ListGrid_1.md#attr-listgridshowfiltereditor), what formItem should be used to display large set of options to the user?

This property is consulted when generating the item for a field with a valueMap containing more options than the [DynamicForm.largeValueMapFilterEditorThreshold](#attr-dynamicformlargevaluemapfiltereditorthreshold). It is also consulted if the item derives its options from an [optionDataSource](FormItem.md#attr-formitemoptiondatasource).

Note that if the [DynamicForm.dataSource](#attr-dynamicformdatasource) does not support the [criterion.operator](../reference.md#type-operatorid) generated by this editor, the editor will not be used. This may occur with the default [SetFilterItem](SetFilterItem.md#class-setfilteritem) if the dataSource does not support `inSet` and `notInSet` criteria.
...
---
## Attr: DynamicForm.showUploadProgress

### Description
If true and the form contains [FileUploadItems](FileUploadItem.md#class-fileuploaditem) with files, [DynamicForm.saveData](#method-dynamicformsavedata) will display upload progress via [FileUploadItem.setProcessingProgress](FileUploadItem.md#method-fileuploaditemsetprocessingprogress).
...
By default `field.editorType` will be used if present - otherwise backs off to deriving the appropriate form item type from the data type of the field (see [formItemTypeSelection](../kb_topics/formItemTypeSelection.md#kb-topic-formitem-classes-for-databound-component-fields) for details).
...
This method is not called for items with [FormItem.canHover](FormItem.md#attr-formitemcanhover) set to false.
...
The returned criteria object can be used to filter data via methods such as [ListGrid.fetchData](ListGrid_2.md#method-listgridfetchdata), [DataSource.fetchData](DataSource_1.md#method-datasourcefetchdata), or, for more advanced usage, [ResultSet.setCriteria](ResultSet.md#method-resultsetsetcriteria).
...
If client-side validation passes, a [DSRequest](../reference_2.md#object-dsrequest) will be sent, exactly as though [DataSource.addData](DataSource_1.md#method-datasourceadddata) or [DataSource.updateData](DataSource_1.md#method-datasourceupdatedata) had been called with ${isc.DocUtils.linkForRef('method:DynamicForm.getValues','the form\\'s values')} as data. The [DSRequest.operationType](DSRequest.md#attr-dsrequestoperationtype) will be either "update" or "add", depending on the current [DynamicForm.saveOperationType](#attr-dynamicformsaveoperationtype).
...
For databound forms, any [DataSource](DataSource_1.md#class-datasource) field validators will be run even if there is no associated item in the form. Validators will also be run on hidden form items. In both these cases, validation failure can be handled via [DynamicForm.handleHiddenValidationErrors](#method-dynamicformhandlehiddenvalidationerrors).
