...
When [fechData()](ListGrid_1.md#method-listgridfetchdata) is called, the automatically created [DSRequest](../reference.md#object-dsrequest) will specify [DSRequest.additionalOutputs](DSRequest.md#attr-dsrequestadditionaloutputs) requesting the field, and any [Criteria](../reference.md#type-criteria) generated by the component will likewise refer to the field from the related DataSource.
...
### Description
When [ListGrid.hiliteIcons](ListGrid_1.md#attr-listgridhiliteicons) are present, where the hilite icon will be placed relative to the field value. See [HiliteIconPosition](../reference.md#type-hiliteiconposition). Overrides [ListGrid.hiliteIconPosition](ListGrid_1.md#attr-listgridhiliteiconposition).
...
### Description
Dictates whether numeric values should be exported as raw numbers instead of formatted values when using [exportClientData()](ListGrid_1.md#method-listgridexportclientdata).
...
### Description
Name of this field. Must be unique within this ListGrid as well as a valid JavaScript identifier - see [FieldName](../reference.md#type-fieldname) for details and how to check for validity.

The name of field is also the property in each record which holds the value for that field.

If a [ListGrid.dataSource](ListGrid_1.md#attr-listgriddatasource) is specified and the DataSource has a field with the same name, the ListGridField and DataSourceField are merged, with any properties on the ListGridField overriding those on the DataSourceField.
...
### Description
Dictates whether the data in this field should be exported raw by [exportClientData()](ListGrid_1.md#method-listgridexportclientdata). If set to true for a field, the values in the field-formatters will not be executed for data in this field. Decreases the time taken for large exports.
...
Note: for databound listGrids, this property may be specified on the [DataSourceField](../reference.md#object-datasourcefield), enabling both client and server side validation.
...
Note that if [ListGridField.hidden](#attr-listgridfieldhidden) is set, or an explicit [ListGridField.showIf](#method-listgridfieldshowif) function exists that will take precedence over this setting. Similarly, an explicit call to [ListGrid.showField](ListGrid_2.md#method-listgridshowfield) or [ListGrid.hideField](ListGrid_1.md#method-listgridhidefield) will clear this setting.
...
### Description
With the [FilterEditor](ListGrid_1.md#attr-listgridshowfiltereditor) showing, the [Operator](../reference_2.md#object-operator) to use when matching values for this field.

Note that you can set all FilterEditor fields to default to either substring or exact match via [autoFetchTextMatchStyle](ListGrid_1.md#attr-listgridautofetchtextmatchstyle), but if you want a mix of exact vs substring match on different fields, you need to use this property, and your ListGrid will produce [AdvancedCriteria](../reference.md#object-advancedcriteria) rather than the simpler [Criteria](../reference.md#type-criteria) format. This is automatically and transparently handled by the SmartClient Server's SQLDataSource and HibernateDataSource in Power Edition or above, but if you have your own filtering implementation, it will need to be able to handle AdvancedCriteria.
...
- [ListGrid.getHeaderContextMenuItems](ListGrid_1.md#method-listgridgetheadercontextmenuitems)
...
### Description
This property is a mapping from data values for this field to [urls](../reference_2.md#type-scimgurl) for icons to display for those data values.
...
If a simple value-to-URL mapping is not enough, you can override [ListGrid.getValueIcon](ListGrid_1.md#method-listgridgetvalueicon) to customize the behavior. You can even specify an empty `valueIcons` map and use [ListGrid.getValueIcon](ListGrid_1.md#method-listgridgetvalueicon) to return arbitrary icons with no fixed mapping.
...
Advanced applications that wish to save formulas separately from a grid's [viewState](ListGrid_2.md#method-listgridgetviewstate) can provide a [UserFormula](../reference.md#object-userformula) as part of the field definition, and may subsequently access the formula is updated via the [ListGrid.formulaUpdated](ListGrid_1.md#method-listgridformulaupdated) notification.
...
### Description
Specifies vertical alignment in the column header for a [rotated title](#attr-listgridfieldrotatetitle): "top", "center", or "bottom". If unset, default behavior is derived from [ListGrid.headerTitleVAlign](ListGrid_1.md#attr-listgridheadertitlevalign).
...
### Description
This value is managed by the menu item titled ["Filter using"](ListGrid_1.md#attr-listgridfilterusingtext) in the [headerContextMenu](ListGrid_1.md#attr-listgridshowheadercontextmenu) that appears when [allowFilterOperators](ListGrid_1.md#attr-listgridallowfilteroperators) has been set to true.

If you need to reset this filter operator you should call [listGrid.setFieldProperties](ListGrid_2.md#method-listgridsetfieldproperties), as in this example:

```
    listGrid.setFieldProperties(fieldName, {operator: null});
```
Note that this property is not listed as "initializable" and is not intended as a means of applying a default operator to the field. Rather, it simply reflects any non-default operator already applied to menu item ["Filter using"](ListGrid_1.md#attr-listgridfilterusingtext). Use [ListGridField.filterOperator](#attr-listgridfieldfilteroperator) to specify the default operator for the field.
...
This attribute specifies how the summary field value will be calculated. See [RecordSummaryFunction](../reference.md#type-recordsummaryfunction) for valid options.
...
### Description
Set this property to true to suppress showing this field on tablets (tablet-sized devices). To update this property at runtime use [ListGrid.setHideOnTablet](ListGrid_1.md#method-listgridsethideontablet)
...
### Description
Derive a [ValueMap](../reference.md#type-valuemap) by fetching records from another DataSource and extracting the [valueField](#attr-listgridfieldvaluefield) and [displayField](#attr-listgridfielddisplayfield) in the loaded records, to derive one valueMap entry per record loaded from the optionDataSource.

Unlike the similar use of [PickList.optionDataSource](PickList.md#attr-picklistoptiondatasource) for [pickLists](../reference.md#interface-picklist) used during editing or filtering, `listGridField.optionDataSource` causes the **entire set of records from the optionDataSource to be fetched**, without paging. Hence listGridField.optionDataSource is appropriate only for smaller valueMaps, and in this situation, [DataSource.cacheAllData](DataSource.md#attr-datasourcecachealldata) may be a better choice, since it creates a write-through cache usable across all components.
...
### Description
When a user requests column autofitting via the [header contextMenu](ListGrid_1.md#method-listgridgetheadercontextmenuitems) or via a [mouse gesture](ListGrid_1.md#attr-listgridheaderautofitevent), what autofit approach is used. If set, this setting overrides the autoFitWidthApproach specified at the ListGrid level.
...
Advanced applications that wish to save summaries separately from a grid's [viewState](ListGrid_2.md#method-listgridgetviewstate) can provide a [UserSummary](../reference_2.md#object-usersummary) as part of the field definition, and may subsequently access the summary is updated via the [ListGrid.summaryUpdated](ListGrid_2.md#method-listgridsummaryupdated) notification.
...
If unset, display format may be set at the listGrid level via [ListGrid.dateFormatter](ListGrid_1.md#attr-listgriddateformatter) or [ListGrid.timeFormatter](ListGrid_1.md#attr-listgridtimeformatter).

If this field is editable the displayFormat will also be passed to the editor created to edit this field. For dates you may also need to set [ListGridField.inputFormat](#attr-listgridfieldinputformat).

### See Also

- [ListGridField.inputFormat](#attr-listgridfieldinputformat)
- [ListGrid.dateFormatter](ListGrid_1.md#attr-listgriddateformatter)
- [ListGrid.timeFormatter](ListGrid_1.md#attr-listgridtimeformatter)
...
[ListGrid.canHover](ListGrid_1.md#attr-listgridcanhover) can be set to true to cause hovers to be shown for all fields by default. In this case, `field.showHover` can be set to false to suppress hovers for an individual field.

All hovers can be disabled, regardless of other settings, by setting [ListGrid.showHover](ListGrid_1.md#attr-listgridshowhover) to false.
...
### Description
A title for this field, to display in the header for the field and in other contexts such as the [menu for picking visible fields](ListGrid_1.md#attr-listgridcanpickfields).

Note: if you want to use HTML tags to affect the display of the header, you should do so via [ListGridField.headerTitle](#attr-listgridfieldheadertitle) instead so that other places where the title appears in the UI are not affected. For example, you might set `headerTitle` to an empty string to suppress the header title on a narrow column, but you would retain the normal title in the `title` property to avoid a blank menu item in the field picker menu, [hilite editor](DataBoundComponent.md#method-databoundcomponentedithilites) and other contexts.

Alternately you can specify a [ListGridField.getFieldTitle](#method-listgridfieldgetfieldtitle) method on the field to return the HTML for the field title.
...
- [ListGridField.getFieldTitle](#method-listgridfieldgetfieldtitle)
...
The [ListGrid.fetchDelay](ListGrid_1.md#attr-listgridfetchdelay) governs the delay in milliseconds between the user editing the filter editor value, and the new filter being applied to the grid.
...
If not explicitly specified, see [ListGrid.getIconCursor](ListGrid_1.md#method-listgridgeticoncursor) for how `"icon"` fields determine icon image cursors, and [ListGrid.getValueIconCursor](ListGrid_2.md#method-listgridgetvalueiconcursor) for how value icon image cursors are determined.
...
- [ListGrid.emptyCellValue](ListGrid_1.md#attr-listgridemptycellvalue)
...
To change this property after fields have been passed to [ListGrid.setFields](ListGrid_2.md#method-listgridsetfields), use [ListGrid.setFieldCellIcon](ListGrid_1.md#method-listgridsetfieldcellicon).
...
### Description
For fields with an [ListGridField.optionDataSource](#attr-listgridfieldoptiondatasource), where [ListGridField.autoFetchDisplayMap](#attr-listgridfieldautofetchdisplaymap) is true, this property will govern the `textMatchStyle` attribute of the [DSRequest](../reference.md#object-dsrequest) parameter passed to [DataSource.fetchData](DataSource.md#method-datasourcefetchdata) when retrieving the remote data set to be used as a basis for this field's valueMap.
...
If present, this method will be invoked instead of [ListGridField.formatCellValue](#method-listgridfieldformatcellvalue) in cases where the grid is rendering non-interactive content outside. See [ListGrid.formatInactiveCellValue](ListGrid_1.md#method-listgridformatinactivecellvalue) for more details.
...
- [ListGrid.formatInactiveCellValue](ListGrid_1.md#method-listgridformatinactivecellvalue)
...
The value of "this" within the method will by the [field definition](../reference.md#object-listgridfield).
...
### Description
A stringMethod which returns the cell value to display for this field for some record. If defined, called by ListGrid.getCellValue(). Called in the scope of the field object. Deprecated as of Jan 12 05 in favor of [ListGridField.formatCellValue](#method-listgridfieldformatcellvalue), because 'getCellValue()' is a lower-level API which handles (for example) returning the HTML for editors within the cell.

### Parameters

| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| viewer | [ListGrid](#type-listgrid) | false | — | the ListGrid for which we're returning a cellValue |
| record | [Object](../reference_2.md#type-object) | false | — | the current record object |
| recordNum | [number](#type-number) | false | — | row-index of the current record |
| field | [ListGridField](#type-listgridfield) | false | — | current field object |
| fieldNum | [number](#type-number) | false | — | column-index of the current field |
| value | [Any](#type-any) | false | — | unformatted value for this field, determined via ListGrid.getRawCellValue() |

### Returns

`[Any](#type-any)` — value to display in the ListGrid cell
...
- [ListGrid.getCellValue](ListGrid_2.md#method-listgridgetcellvalue)
...
### Description
An optional [stringMethod](../reference.md#kb-topic-string-methods-overview) which if provided, is evaluated to conditionally determine whether this field should be displayed. Evaluated on initial draw, then reevaluated on explicit calls to `listGrid.refreshFields()` or `listGrid.setFields()`.
...
| context | [ListGrid](#type-listgrid) | false | — | A pointer back to the list grid displaying this field will be available as the `context` argument. Note that you can also get a pointer to the field definition object by calling `context.getField(fieldName)` |
...
`[ValueMap](../reference.md#type-valuemap)` — ValueMap for the field (or null if no valueMap required)
...
You can use [setFieldProperties()](ListGrid_2.md#method-listgridsetfieldproperties) to dynamically update the title.
