...
### Description
Horizontal offset of a [ListGrid.headerMenuButton](#attr-listgridheadermenubutton) from its parent button's trailing edge (right in LTR, left in RTL). Positive values move the menu button to the right.
...
Another special value is `"*inexpensive*"`: this requests all visible, primary-key, and declaratively required fields (same as `"*visible*"`) but also tells the server to automatically include any additional fields that are not considered expensive to fetch, via [DSRequest.addInexpensiveFields](DSRequest.md#attr-dsrequestaddinexpensivefields). The server-side `DataSource.isExpensive()` method determines which fields are expensive — see [DSRequest.addInexpensiveFields](DSRequest.md#attr-dsrequestaddinexpensivefields) for the default heuristics and the [DataSourceField.expensive](DataSourceField.md#attr-datasourcefieldexpensive) attribute for per-field overrides.
...
### Description
Controls how framework-generated icons are handled in the canvas [ListGrid.preRender](#attr-listgridprerender) preview. See [PreRenderImageMode](../reference_2.md#type-prerenderimagemode) for available modes.
...
A number of builtin modes are provided - see [ListGrid.hoverMode](ListGrid_2.md#attr-listgridhovermode). Note, if a `hoverMode` is set but `showHoverComponents` is left null, it will default to true.
...
To limit fetched data to just the visible fields and fields required for declared formulas, see [ListGrid.fetchFields](#attr-listgridfetchfields).
...
- [PreRenderImageMode](../reference_2.md#type-prerenderimagemode)
...
### Description
The [CanvasGrid](CanvasGrid.md#class-canvasgrid) AutoChild created when [ListGrid.preRender](#attr-listgridprerender) is "structure" or "data". Positioned behind the grid body to show a canvas-based preview while the DOM renders.
...
### Description
Default properties for the [CanvasGrid](CanvasGrid.md#class-canvasgrid) AutoChild used when [ListGrid.preRender](#attr-listgridprerender) is set to "structure" or "data". The CanvasGrid renders an HTML5 Canvas preview layer behind the main grid body.
...
When `false` (the default), `getDefaultFormattedValue()` applies complete formatting including [ListGridField.valueMap](ListGridField.md#attr-listgridfieldvaluemap) mapping, [ListGridField.displayField](ListGridField.md#attr-listgridfielddisplayfield) substitution, [ListGridField.cellValueTemplate](ListGridField.md#attr-listgridfieldcellvaluetemplate) evaluation, [ListGridField.format](ListGridField.md#attr-listgridfieldformat) application, and type-specific formatters, matching the behavior of normal cell rendering.

Set to `true` to revert to legacy behavior where `getDefaultFormattedValue()` returned minimally formatted values without these transformations. This preserves backward compatibility with existing code that relies on the legacy behavior.
...
---
## Attr: ListGrid.inlineEditItemGap

### Description
Pixel gap subtracted from each column's edit item width when inline editors are in use. Prevents rounded inline items from visually touching at column boundaries.
...
---
## Attr: ListGrid.useInlineFilterEditors

### Description
When true, filter editors resolve to the Inline\* variant of their standard editor type when such a class exists (e.g. TextItem → InlineTextItem). Set by `load_skin.js` in skins that use rounded, density-scaled inline form items in grids.
...
Grids with [canPickOmittedFields:true](#attr-listgridcanpickomittedfields) will only show the explicitly specified set of fields, but will similarly show them in the order in which they're defined within the dataSource. To limit fetched data to just the visible fields and fields required for declared formulas, see [ListGrid.fetchFields](#attr-listgridfetchfields).
...
- [ListGrid.fetchFields](#attr-listgridfetchfields)
...
---
## Attr: ListGrid.autoFitExpandLengthThreshold

### Description
When [ListGrid.autoFitFieldWidths](#attr-listgridautofitfieldwidths) causes the grid to choose a field to expand, fields with [ListGridField.length](ListGridField.md#attr-listgridfieldlength) below this threshold are skipped in favor of fields with longer or unspecified length.
...
---
## Attr: ListGrid.useInlineEditors

### Description
When true, inline editing editors resolve to the Inline\* variant of the standard editor type. See [ListGrid.useInlineFilterEditors](#attr-listgriduseinlinefiltereditors) for filter editors.
...
**Styling differences:** In divGrid mode, [GridRenderer.cellPadding](GridRenderer.md#attr-gridrenderercellpadding) is not applied via HTML attributes (since DIVs don't support CELLPADDING). Cell padding must be defined in the [ListGrid.baseStyle](#attr-listgridbasestyle) CSS class. Vertical row spacing can also be achieved via `tableStyle` CSS using `border-collapse:separate` with `border-spacing`. **Note:** Since divGrid mode is now enabled by default, applications that relied on the default 2px `cellPadding` in TABLE mode may need to add explicit padding to their `baseStyle` CSS.
