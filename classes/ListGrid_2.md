# ListGrid Documentation (Part 2 of 3)
...
Note that [ListGrid.groupByMaxRecords](ListGrid_1.md#attr-listgridgroupbymaxrecords) must be set at least as high as [ListGrid.groupByAsyncThreshold](#attr-listgridgroupbyasyncthreshold) or asynchronous grouping will never be used.

During async grouping, interactivity is blocked and the [ListGrid.asynchGroupingPrompt](ListGrid_1.md#attr-listgridasynchgroupingprompt) is shown to the user, then hidden when grouping completes; [ListGrid.groupByComplete](#method-listgridgroupbycomplete) then fires.
...
- [ListGrid.booleanTrueImage](ListGrid_1.md#attr-listgridbooleantrueimage)
...
### Description
Warning shown to the user when a grouping attempt failed as the data set length exceeds [ListGrid.groupByMaxRecords](ListGrid_1.md#attr-listgridgroupbymaxrecords).

If defined, this prompt will be shown to the user in a [warning dialog](isc.md#staticmethod-iscwarn) when the user attempts to group a data set that exceeds the [ListGrid.groupByMaxRecords](ListGrid_1.md#attr-listgridgroupbymaxrecords) threshold.

This can occur if an already-grouped grid's filter criteria are modified such that a new set of records is loaded from the DataSource which exceeds the [ListGrid.groupByMaxRecords](ListGrid_1.md#attr-listgridgroupbymaxrecords) threshold.

It can also occur when a user attempts to group a grid with a partially loaded data set where the true size of the data set is not known due to [progressiveLoading](DataBoundComponent.md#attr-databoundcomponentprogressiveloading). In this case, the grouping logic will attempt to retrieve all the records in the data set and may get back a new total row count from the DataSource which exceeds [ListGrid.groupByMaxRecords](ListGrid_1.md#attr-listgridgroupbymaxrecords).

In either case the warning will be displayed to the user and the [group by menu item](ListGrid_1.md#attr-listgridcangroupby) will be disabled.

See also [ListGrid.disabledGroupByPrompt](ListGrid_1.md#attr-listgriddisabledgroupbyprompt).
...
### Description
Should the [summary row](ListGrid_1.md#attr-listgridshowgridsummary) or [group summaries](#attr-listgridshowgroupsummary) be updated during editing of grid records? This can be set false to improve performance when a large number of [ListGridFields](../reference_2.md#object-listgridfield) or [DataSourceFields](../reference_2.md#object-datasourcefield) are present for the grid.

Note that summaries will always be updated when the edits are saved, so to avoid recalculation overhead when a row or cell edit is completed, you must also set [ListGrid.autoSaveEdits](ListGrid_1.md#attr-listgridautosaveedits): false. Summaries will then be updated upon your manual save, such as [ListGrid.saveAllEdits](#method-listgridsavealledits).

### See Also

- [ListGrid.recalculateSummaries](#method-listgridrecalculatesummaries)
...
### Description
Scaling factor to translate from ListGrid field widths in pixels to Excel/OpenOffice units for field width, which are 1/256th of the width of the widest digit character in the default font for the spreadsheet. See [ListGrid.exportFieldWidths](ListGrid_1.md#attr-listgridexportfieldwidths) for where this is used.
...
### Description
When [canExpandRecords](ListGrid_1.md#attr-listgridcanexpandrecords) is true and [expansionMode](ListGrid_1.md#attr-listgridexpansionmode) is _editor_, whether a dialog should be displayed when an expanded row is collapsed while it's nested editor has changed values.
...
### Description
Properties to apply to the sorter button. Overrides defaults applied via [ListGrid.sorterDefaults](ListGrid_1.md#attr-listgridsorterdefaults).
...
### Description
Default configuration properties for the "remove field" displayed when [ListGrid.canRemoveRecords](ListGrid_1.md#attr-listgridcanremoverecords) is enabled. [Class.changeDefaults](Class.md#classmethod-classchangedefaults) should be used when modifying this object.

The default configuration includes a [ListGridField.recordClick](ListGridField.md#method-listgridfieldrecordclick) handler which calls [ListGrid.removeData](#method-listgridremovedata) to actually perform the data removal.
...
### Description
Get the fully formatted value for a cell in this grid, including all custom formatters.

This method applies the complete formatting pipeline that would be used when rendering a cell in the grid, including:

*   [ListGridField.valueMap](ListGridField.md#attr-listgridfieldvaluemap) mapping
*   [ListGridField.displayField](ListGridField.md#attr-listgridfielddisplayfield) substitution
*   Custom [ListGridField.formatCellValue](ListGridField.md#method-listgridfieldformatcellvalue) or [ListGrid.formatCellValue](#method-listgridformatcellvalue) methods
...
If this component is [databound](DataBoundComponent.md#method-databoundcomponentsetdatasource) and not all records that match the current [filter-criteria](ListGrid_3.md#method-listgridgetcriteria) have [been loaded](ResultSet.md#method-resultsetallmatchingrowscached), you can call [ListGrid.loadAllRecords](#method-listgridloadallrecords) - this method accepts a callback which is fired when all necessary data has arrived. When that callback fires, a call to `exportClientData` will have access to the full dataset for the filter.
...
| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| styleName | [CSSStyleName](../reference_2.md#type-cssstylename) | false | — | new body style name |
...
| newStyle | [CSSStyleName](../reference_2.md#type-cssstylename) | false | — | new titleTyle for the field header |
...
|------|------|----------|---------|-------------|
| name | [String](#type-string) | false | — | name of the headerSpan, as specified via [HeaderSpan.name](HeaderSpan.md#attr-headerspanname). |
| newTitle | [CSSStyleName](../reference_2.md#type-cssstylename) | false | — | new titleStyle for the headerSpan |
...
setUserSummary() may not be used to add a `userSummary` to a field of this grid that did not already have one (i.e. a field may not be converted to a summary field).
...
Grouping is often an asynchronous operation, both because of automatic loading of remaining rows, and because asynchronous processing is required to work around bugs in some browsers related to misdetection of "hung" scripts (see [ListGrid.groupByAsyncThreshold](#attr-listgridgroupbyasyncthreshold)). Define a [ListGrid.handleGroupBy](#method-listgridhandlegroupby) method to be called when grouping is about to start (potentially canceling it), and define a [ListGrid.groupByComplete](#method-listgridgroupbycomplete) method to be notified when it's done.
...
### Description
Return the pointer to a particular record by record number. Synonym for [ListGrid.getCellRecord](ListGrid_3.md#method-listgridgetcellrecord).
...
- [ListGrid.getCellRecord](ListGrid_3.md#method-listgridgetcellrecord)
...
`[ListGridGroupState](../reference_2.md#type-listgridgroupstate)` — current view state for the grid.
...
- [ListGridGroupState](../reference_2.md#type-listgridgroupstate)
...
---
## Method: ListGrid.setFieldAlign

### Description
Updates [ListGridField.align](ListGridField.md#attr-listgridfieldalign) for the specified field and refreshes the header button and body cells to reflect the new alignment.

### Parameters

| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| fieldNum | [int](../reference.md#type-int)|[FieldName](../reference.md#type-fieldname) | false | — | name of the field, or index. |
| align | [Alignment](../reference_2.md#type-alignment) | false | — | The new field alignment. |
...
---
## Method: ListGrid.preRenderDone

### Description
Notification method fired when the canvas [ListGrid.preRender](ListGrid_1.md#attr-listgridprerender) preview has finished painting and is visible to the user. At this point the user sees the canvas preview — the real HTML body has not yet rendered.

Fires on both the initial draw and every subsequent scroll-triggered re-render where the canvas preview is repainted.

Only fires when [ListGrid.preRender](ListGrid_1.md#attr-listgridprerender) is `"data"` or `"structure"`. Does not fire when preRender is `"none"`.

To detect when the real HTML grid body replaces the canvas preview, use [ListGrid.preRenderHTMLReplaced](#method-listgridprerenderhtmlreplaced) instead.
...
- [ListGrid.preRenderHTMLReplaced](#method-listgridprerenderhtmlreplaced)
...
### Description
Perform a one-time horizontal auto-fit of the fields passed. Fields will be sized to match their contents or title (as specified in [ListGrid.autoFitWidthApproach](ListGrid_1.md#attr-listgridautofitwidthapproach)) Does not establish permanent auto-fitting - use [ListGrid.setAutoFitWidth](ListGrid_3.md#method-listgridsetautofitwidth) to do so.
...
---
## Method: ListGrid.isPreRendering

### Description
Returns `true` if this grid currently has a visible canvas [ListGrid.preRender](ListGrid_1.md#attr-listgridprerender) overlay — that is, the user sees the Canvas 2D preview, not the real HTML body.

Use this method inside custom [formatCellValue()](#method-listgridformatcellvalue), [getGroupTitle()](#method-listgridgetgrouptitle), and similar callbacks to detect when the framework is rendering for the canvas preview. Canvas 2D cannot display HTML, so callbacks should return plain text (no tags) when this method returns true.

Returns `false` when [ListGrid.preRender](ListGrid_1.md#attr-listgridprerender) is `"none"`, when the canvas preview has not yet been shown, or after the real HTML body has replaced the canvas overlay.

### Returns

`[Boolean](#type-boolean)` — true if the canvas preRender overlay is currently visible
...
### Description
Shows an additional field near the beginning of the field list (after any [row number](#attr-listgridshowrownumbers) field) that can be dragged to drag the current selection. This feature is useful in [touch environments](Browser.md#classattr-browseristouch) where both touch scrolling and dragging are needed on the same grid, and allows scrolling to be triggered on the other fields so that both operations are available. Targeted touch environments include both mobile devices, and Windows hardware that supports [Dual Input Mode](Browser.md#classattr-browsersupportsdualinput) such as Microsoft Surface.
...
setUserFormula() may not be used to add a `userFormula` to a field of this grid that did not already have one (i.e. a field may not be converted to a formula field).
...
Note that returning false from this method will not prevent any specified [ListGrid.rowClick](ListGrid_3.md#method-listgridrowclick) handler from firing.
...
*   This method is called from the default implementation of [ListGrid.rowClick](ListGrid_3.md#method-listgridrowclick), so if that method is overridden this method may not be fired.
...
- [ListGrid.rowClick](ListGrid_3.md#method-listgridrowclick)
...
---
## Method: ListGrid.preRenderHTMLReplaced

### Description
Notification method fired each time the real HTML grid body replaces the canvas [ListGrid.preRender](ListGrid_1.md#attr-listgridprerender) preview. This means the DOM body has fully rendered and the canvas overlay has been hidden — the user is now looking at the real grid. All grid APIs that depend on rendered HTML ([ListGrid.getCellPageRect](#method-listgridgetcellpagerect), [ListGrid.getEditFormItem](#method-listgridgeteditformitem), etc.) are available at this point.

Fires on both the initial draw and every subsequent scroll-triggered re-render where the canvas preview was shown first.

This method fires slightly after the transition is visually complete (deferred by one event-loop turn) to ensure the body's scroll position has stabilized.

Only fires when [ListGrid.preRender](ListGrid_1.md#attr-listgridprerender) is `"data"` or `"structure"`. Does not fire when preRender is `"none"`.

For grids that do not use [ListGrid.preRender](ListGrid_1.md#attr-listgridprerender), the standard [drawn()](Canvas.md#method-canvasdrawn) notification indicates when the grid's HTML has rendered. When preRender IS enabled, `drawn()` fires when the canvas preview is first shown (before the HTML body), so use this method instead to detect the HTML transition.
...
- [ListGrid.preRender](ListGrid_1.md#attr-listgridprerender)
- [ListGrid.preRenderDone](#method-listgridprerenderdone)
...
|------|------|----------|---------|-------------|
| name | [String](#type-string) | false | — | name of the field. |
| newStyle | [CSSStyleName](../reference_2.md#type-cssstylename) | false | — | new baseStyle for the field header |
...
|------|------|----------|---------|-------------|
| name | [String](#type-string) | false | — | name of the headerSpan, as specified via [HeaderSpan.name](HeaderSpan.md#attr-headerspanname). |
| newStyle | [CSSStyleName](../reference_2.md#type-cssstylename) | false | — | new baseStyle for the headerSpan |
...
### Description
Reset this grid's grouping to match the [ListGridGroupState](../reference_2.md#type-listgridgroupstate) object passed in.  
...
| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| groupState | [ListGridGroupState](../reference_2.md#type-listgridgroupstate) | false | — | Object describing the desired grouping state of the grid |
...
If you wish to access the `criteria` applied to the grid without picking up any edits to the Filter Editor, use [ListGrid.getCriteria](ListGrid_3.md#method-listgridgetcriteria) instead.
...
### Description
Identifies whether the passed-in field is the specially generated [rowNumberField](ListGrid_1.md#attr-listgridrownumberfield) used when [ListGrid.showRowNumbers](#attr-listgridshowrownumbers) is true. Use this method in your custom event handlers to avoid inappropriately performing actions when the rowNumberField is clicked on.
...
---
## Method: ListGrid.setFieldCellAlign

### Description
Updates [ListGridField.cellAlign](ListGridField.md#attr-listgridfieldcellalign) for the specified field and refreshes body cells to reflect the new alignment.
...
|------|------|----------|---------|-------------|
| fieldNum | [int](../reference.md#type-int)|[FieldName](../reference.md#type-fieldname) | false | — | name of the field, or index. |
| cellAlign | [Alignment](../reference_2.md#type-alignment) | false | — | The new alignment for the field's cells. |
...
| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| styleName | [CSSStyleName](../reference_2.md#type-cssstylename) | false | — | new body style name when showing alternateRecordStyles |
...
Does not establish permanent auto-fitting - use [ListGrid.setAutoFitWidth](ListGrid_3.md#method-listgridsetautofitwidth) or [ListGrid.setAutoFitFieldWidths](#method-listgridsetautofitfieldwidths) to do so.
...
Note that developers may wish to use [ListGrid.selectRange](ListGrid_3.md#method-listgridselectrange) to select a single contiguous range.
...
- [ListGrid.editorEnter](ListGrid_3.md#method-listgrideditorenter)
