...
Static mail server to use for sending emails can be configured in the [server.properties](../reference.md#kb-topic-serverproperties-file) file. The following values can be provided:
...
### Description
Allows you to specify an alternative source of substitution values for the templating process (by default, the data record(s) returned by the DataSource operation are used). If set, this [VelocityExpression](../reference.md#type-velocityexpression) should evaluate to a Java `Map` object, or a Java `List` containing only `Map`s.
