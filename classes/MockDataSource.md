...
### Description
A special kind of [client-only DataSource](DataSource_1.md#attr-datasourceclientonly) that can be configured with ["mock data"](#attr-mockdatasourcemockdata) - a simple text format for table or tree data.
...
`MockDataSource` is primarily intended as a temporary form of DataSource used during the process of converting a mockup into a real application. Generally, if creating a client-only DataSource in JavaScript , there is no reason to use the mock data format, as the mock data is not especially readable when written as a String literal. The mock data format _can_ be a slightly more compact and readable as compared to declaring [DataSource.cacheData](DataSource_1.md#attr-datasourcecachedata) in XML.

Note: If a MockDataSource has [addGlobalId](DataSource_1.md#attr-datasourceaddglobalid) set to true, it will be made available in global scope.
