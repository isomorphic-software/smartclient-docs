...
*   `criteria.*` prefix: references a value from the client's request criteria. For example, `valuePath="criteria.originCity"` resolves to the value the client submitted for `originCity` and uses it as a constraint on the binding's target field. When the client sends AdvancedCriteria with the same field at multiple values in different OR branches, the server preserves the client's AND/OR structure (field-name remapping), producing order-independent SQL.
*   `submitted.*` prefix: references a value from the client's submitted record (`dsRequest.getValues()`).
...
The `criteria.*` prefix can inspect the client's request criteria: `criteria.startDate notNull` rejects if the client did not supply a `startDate` criterion. The `submitted.*` prefix inspects the client-submitted values.
