...
`operationBinding.criteria` also supports [ServerDynamicCriteria](../reference_2.md#type-serverdynamiccriteria) features, including:
...
Note that `requiresWhen` is purely an access control check. To declare non-security criteria that constrain the request, use [OperationBinding.criteria](#attr-operationbindingcriteria).
