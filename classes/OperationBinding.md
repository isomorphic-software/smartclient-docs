...
### Description
Comma-separated list of JavaScript files to pre-load into a pooled server-side JavaScript context for this OperationBinding's [server-side scriptlet](#attr-operationbindingscript) or [server-side process](#attr-operationbindingprocess).

Files are evaluated once when each pooled context is created. Contexts are created lazily as requests need them, then reused across requests, eliminating the repeated overhead of loading common modules or libraries.
...
The server canonicalizes the order of `contextFiles` before identifying the shared context pool: `ISC_Core_Server.js` (if present) is loaded first, then other SmartClient Server modules in the order first added, then application-specific files in the order first added. Duplicate files are ignored. For example, these configurations are equivalent and share a pool:

```
   contextFiles="databinding, app/calculations.js"
   contextFiles="app/calculations.js, databinding"
```
These are also equivalent because `"databinding"` first includes `"core"`:
```
   contextFiles="core, databinding"
   contextFiles="databinding, core"
   contextFiles="databinding"
```
Application-specific file order is still significant. These configurations are not equivalent and do not share a pool:
```
   contextFiles="databinding, app/base.js, app/customGlobals.js"
   contextFiles="databinding, app/customGlobals.js, app/base.js"
```
Likewise, adding another application-specific context file creates a different configuration and a separate pool:
```
   contextFiles="databinding, app/calculations.js"
   contextFiles="databinding, app/calculations.js, app/customGlobals.js"
```

When listing application-specific files, include an appropriate SmartClient shortcut such as `"databinding"`, so SmartClient server APIs are available before those files are loaded.

When `contextFiles` is specified, context pooling is automatically enabled because [useContextFiles](#attr-operationbindingusecontextfiles) defaults to true. When a context file changes, cached contexts created from the previous file contents are discarded rather than reused. A context already executing may complete its current request, but it is closed when returned to the pool.

**Note:** Pooled contexts are **not** cleared between requests. `contextFiles` should define reusable functions or modules, and [operationFiles](#attr-operationbindingoperationfiles) or the scriptlet should overwrite any request-handling-specific globals or state they create or use. Conversely, `operationFiles` and the scriptlet should **not** overwrite shared globals initialized by `contextFiles`, because `contextFiles` are not re-run after the context is created. If there is a need to customize the shared globals initialized by `contextFiles`, add such customization as another context file.

**Pool sharing:** Pooled contexts are created for each canonically distinct `contextFiles` configuration, not for a particular DataSource, OperationBinding, operationId, scriptlet, or [operationFiles](#attr-operationbindingoperationfiles) setting. Thus, different OperationBindings, even on different DataSources, may reuse the same context to execute their scripts if they have the same canonical `contextFiles` configuration. This is why request-handling-specific globals or state **must** be overwritten by `operationFiles` or the scriptlet, and why operations that require separate context pools should use canonically distinct `contextFiles` configurations.
...
---
## Attr: OperationBinding.filterBeforeGrouping

### Description
When true, all criteria for a [serverSummaries](../kb_topics/serverSummaries.md#kb-topic-server-summaries) request are applied **before** aggregation (SQL `WHERE`) rather than being automatically split between `WHERE` and `HAVING`. In this mode, [afterWhereClause](#attr-operationbindingafterwhereclause) is not auto-generated.
...
**NOTE:** when this flag is true, client-side filtering will not match server-side filtering for aggregated fields, because the client always filters post-aggregation. You can set [ResultSet.useClientFiltering](ResultSet.md#attr-resultsetuseclientfiltering) to disable client-side filtering via [ListGrid.dataProperties](ListGrid_1.md#attr-listgriddataproperties), or mark individual fields [canFilter:false](ListGridField.md#attr-listgridfieldcanfilter).

For example, with `filterBeforeGrouping:true` and the "avg" function on a "price" field, criteria "price < 5" eliminates records where price is less than 5 _before_ the average is calculated.
...
- [OperationBinding.afterWhereClause](#attr-operationbindingafterwhereclause)
- [DSRequest.filterBeforeGrouping](DSRequest.md#attr-dsrequestfilterbeforegrouping)
...
`operationBinding.criteria` also supports [ServerDynamicCriteria](../reference_2.md#type-serverdynamiccriteria) features (see the **Choosing the right mechanism** section of [ServerDynamicCriteria](../reference_2.md#type-serverdynamiccriteria) for a comparison with [OperationBinding.requiresWhen](#attr-operationbindingrequireswhen) and other server-side customization options), including:

*   `auth.*` / `context.*` references: evaluated on the server — `auth.*` leaves that evaluate to true are stripped (no restriction), leaves that evaluate to false produce a `'0'='1'` clause (zero rows for that branch).
*   `criteria.*` prefix: references a value from the client's request criteria and adds it as a server- enforced constraint, tightening the result set beyond what the client requested.
*   `values.*` prefix: references a value from the client's submitted record (`dsRequest.getValues()`).
*   `storedRecord.*` / `mergedRecord.*` prefixes: reference the pre-modification DB record or stored + submitted overlay (see [ServerDynamicCriteria](../reference_2.md#type-serverdynamiccriteria) for details).
*   Relational references via dot-notation (e.g. `Order.status`) are automatically converted to SQL subqueries by the server pipeline.
*   Aggregation shorthands (`#count`, `#sum`, etc.) are expanded before SQL generation.

For back-compatibility and additional flexibility, criteria values are also evaluated through Velocity, so standard Velocity variables (`$session`, `$criteria`, `$values`, `$responseData`, etc.) work as well. This can be controlled via [OperationBinding.velocityInCriteriaEnabled](#attr-operationbindingvelocityincriteriaenabled).
...
```
Example using `criteria.*` prefix for field mapping (client sends `searchRegion`, server constrains `assignedRegion`):
```
 <criteria fieldName="assignedRegion" operator="equals"
           valuePath="criteria.searchRegion"/>
...
- [OperationBinding.velocityInCriteriaEnabled](#attr-operationbindingvelocityincriteriaenabled)
...
This setting is used with [OperationBinding.contextFiles](#attr-operationbindingcontextfiles). Unlike `contextFiles`, which are evaluated when a pooled context is created and then retained in that context, operation files are executed fresh on each request. Use this for per-request logic like loading request-specific data or invoking calculations with that data. This setting only has an effect when the pooled Graal context path is active, which requires `contextFiles`.
...
*   Prepare request-specific values or globals that the inline OperationBinding script then uses or returns

The direct return value of an operation file is ignored.
...
### Description
Declares a server-side workflow [Process](Process.md#class-process) to execute for this operation, replacing the default CRUD operation entirely (same semantics as [OperationBinding.script](#attr-operationbindingscript) or a [DMI](../reference_2.md#object-serverobject)).
...
Use [OperationBinding.contextFiles](#attr-operationbindingcontextfiles) to control which SmartClient server modules are pre-loaded into the JavaScript context pool (default: `"databinding"` which loads ISC\_Core\_Server.js and ISC\_DataBinding\_Server.js).
...
### Description
[ServerDynamicCriteria](../reference_2.md#type-serverdynamiccriteria) evaluated server-side to determine whether the current operation is allowed. ServerDynamicCriteria can inspect any part of the DSRequest and compare the inbound data to fixed values or to anything that can be fetched from related DataSources, so this is a very powerful and flexible mechanism, which can be fully visually edited in Reify.

For simpler checks, consider [OperationBinding.requiresRole](#attr-operationbindingrequiresrole) (checks the user's static roles) or [OperationBinding.requires](#attr-operationbindingrequires) (evaluates a Velocity expression).
...
Note that `requiresWhen` is purely an access control check. To declare non-security criteria that constrain the request, use [OperationBinding.criteria](#attr-operationbindingcriteria). For a detailed comparison of all server-side customization mechanisms, see the **Choosing the right mechanism** section of [ServerDynamicCriteria](../reference_2.md#type-serverdynamiccriteria).

`requiresWhen` acts purely as a gate: it either allows the request to proceed or rejects it entirely. It never silently filters rows. **Behavior by operation type:**

*   **fetch**: the criteria are evaluated as a gate. If they don't match, the entire request is rejected with an authorization failure.
*   **add**: the criteria are evaluated against the submitted record values. If they don't match, the add is denied.
*   **update / remove** (single-record): the criteria are evaluated against the _stored server record_ (pre-modification state, fetched from the database). Client-submitted values are never merged into the evaluation record — this prevents a malicious client from submitting values crafted to bypass the gate (e.g. submitting `status:"unlocked"` to pass a `status != "locked"` check). When the `requiresWhen` criteria contains relational references (e.g. `Order.status`) and all referenced DataSources are SQL on the same database, the evaluation is performed as a single SQL query (no N+1 fetches).
*   **update / remove** (with [OperationBinding.allowMultiUpdate](#attr-operationbindingallowmultiupdate)): the criteria are evaluated as a gate against the submitted values and request context. If they don't match, the entire operation is denied.

**Auth/context criteria are evaluated first:** if the `requiresWhen` criteria contains `auth.*` or `context.*` leaves at the top-level AND, they are extracted and evaluated before the remaining criteria. If the auth check fails, the request is rejected immediately without evaluating relational or other criteria (no unnecessary database fetches). Auth leaves inside an OR sub-tree are NOT extracted — they are evaluated together with the other OR branches so the evaluator can short-circuit correctly.

The `criteria.*` prefix can inspect the client's request criteria: `criteria.startDate notNull` rejects if the client did not supply a `startDate` criterion. The `values.*` prefix inspects the client-submitted values. The `storedRecord.*` and `mergedRecord.*` prefixes access the pre-modification DB record and the stored + submitted overlay, respectively (see [ServerDynamicCriteria](../reference_2.md#type-serverdynamiccriteria)).
...
OperationBindings with the same canonical `contextFiles` configuration share a context pool. If a later request for the same pool specifies a larger `contextPoolSize`, the pool size is increased to that larger value and a server warning is logged. Smaller values do not shrink an existing pool.
...
---
## Attr: OperationBinding.velocityInCriteriaEnabled

### Description
Controls whether Velocity template evaluation runs on [OperationBinding.criteria](#attr-operationbindingcriteria) values. When false, `$session`, `$responseData`, and other Velocity variables in criteria values are treated as literal strings rather than being evaluated.

This flag is checked in a 3-tier hierarchy (first non-null wins):

1.  This binding-level property
2.  [DataSource.velocityInCriteriaEnabled](DataSource_1.md#attr-datasourcevelocityincriteriaenabled) (DS level)
3.  `operationBinding.velocityInCriteriaEnabled` in `server.properties` (global, defaults to `true`)

With the introduction of [ServerDynamicCriteria](../reference_2.md#type-serverdynamiccriteria) prefixes (`criteria.*`, `submitted.*`) in `operationBinding.criteria`, many use cases previously requiring Velocity can now be handled declaratively. This flag allows Velocity to be disabled when it is not needed.
...
When true (the default), contexts are returned to a pool after script execution and can be reused by subsequent requests. When false, this OperationBinding uses the normal server-side JavaScript script evaluation path; `contextFiles` and [operationFiles](#attr-operationbindingoperationfiles) are not applied.

Setting this to false disables the performance benefit of context pooling, but may be useful for debugging scenarios where you want to avoid reusing a pooled execution environment.
