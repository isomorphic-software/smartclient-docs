...
### Description
Comma-separated list of user roles that are allowed to invoke the operation described by this operationBinding. If the current user has any of the roles listed, they can invoke the operation. Also note that `authentication.superuserRole` can be specified in the [server.properties](../reference.md#kb-topic-serverproperties-file) file. If set this denotes a "super user" role - any user with that role will have access to all operations, regardless of the "requiresRole" settings for the operation.
...
A list of [DSRequestModifier](../reference.md#object-dsrequestmodifier)s that will be used to modify the criteria of each [DSRequest](../reference.md#object-dsrequest) that uses this operationBinding. Note that the criteria elements are applied to DSRequest criteria as follows:
...
### Description
Additional properties to pass through to the [DSRequest](../reference.md#object-dsrequest) created for this operation. Note that these will be cumulative with and will override on a per-property basis any properties set via [DataSource.requestProperties](DataSource.md#attr-datasourcerequestproperties).
...
### Description
The format in which the data should be exported. Default is "csv". See [ExportFormat](../reference.md#type-exportformat) for more information.
...
A list of [DSRequestModifier](../reference.md#object-dsrequestmodifier)s that will be used to modify the values object of each [DSRequest](../reference.md#object-dsrequest) that uses this operationBinding. See this example: *queuedAdd example*.
...
The format for specifying `methodArguments` is as a comma separated list of VTL (Velocity Template Language) expressions. See the [VTL Reference](http://jakarta.apache.org/velocity/docs/vtl-reference-guide.html) and [Velocity User Guide](http://jakarta.apache.org/velocity/docs/user-guide.html) for an overview of how to use VTL.
...
### Description
Indicates that the specified [VelocityExpression](../reference.md#type-velocityexpression) must be true for a user to access this operationBinding.
...
Overrides the same setting at the [DataSource](DataSource.md#attr-datasourceowneridfield) level.

If ownerIdField is specified, [requiresAuthentication](DataSource.md#attr-datasourcerequiresauthentication) will default to `true`. If `requiresAuthentication` is explicitly set to `false`, then unauthenticated users will be able to see all records. To avoid this, you can use [guestUserId](DataSource.md#attr-datasourceguestuserid) to specify a default user to apply when no one has authenticated.
...
For a dataSource of [serverType](DataSource.md#attr-datasourceservertype) "sql" or "hibernate", this property can be specified on an operationBinding to indicate that the server should run user-specified SQL, rather than the query it would normally generate to satisfy a dataSource operation. This property allows you to provide a fully-customized query; an alternative is to provide custom "pieces" to the query generator, via properties such as [whereClause](#attr-operationbindingwhereclause) and [valuesClause](#attr-operationbindingvaluesclause). See the [customQuerying](../kb_topics/customQuerying.md#kb-topic-custom-querying-overview) for more details.
