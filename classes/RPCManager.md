...
Some browsers now do support cross domain requests through the use of Http Access Control headers (See the [W3C Cross-Origin Resource Sharing recommendation](http://www.w3.org/TR/cors/)). If your application intends to rely on this behavior to perform cross-domain requests, you can set `allowCrossDomainCalls` to true to disable the standard SmartClient warning when such calls occur.
...
### Description
Returns true if a screen with the given name has already been cached by a call to [RPCManager.cacheScreens](#classmethod-rpcmanagercachescreens) (or [RPCManager.loadProject](#classmethod-rpcmanagerloadproject)), false otherwise.
...
You can use a custom skin when exporting your HTML content. To use a custom skin, add a line to [server.properties](../reference.md#kb-topic-serverproperties-file):
...
The special placeholder `{webRoot}` is replaced at runtime with the application's web root directory (as configured via the `webRoot` setting in [server.properties](../reference.md#kb-topic-serverproperties-file)). For example:
...
| globals | [Array of String](#type-array-of-string)|[CreateScreenSettings](#type-createscreensettings) | true | — | widgets to allow to take their global IDs, or a widget remap config |
...
### Description
Loads projects using the [ProjectLoaderServlet](../kb_topics/servletDetails.md#kb-topic-the-core-and-optional-smartclient-servlets), reachable at [RPCManager.projectLoaderURL](#classattr-rpcmanagerprojectloaderurl), and fires the given callback after screens have been [cached](#classmethod-rpcmanagercachescreens).
...
| callback | [Function](#type-function) | false | — | callback for notification of completion of project/s loaded and screens cached. |
...
| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| actionURL | [URL](../reference.md#type-url) | false | — | new actionURL |
...
The ScreenLoaderServlet will look for a file named _screenName_.ui.xml in the directory given by the "project.ui" setting, which defaults _webroot_/shared/ui and can be configured in [server.properties](../reference.md#kb-topic-serverproperties-file).
...
Note: This method only applies to operations submitted via [XMLHttpRequest](../reference_2.md#type-rpctransport) - it is not possible to provide similar error handling for other transports.
