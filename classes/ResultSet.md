...
`recordLimit` exists to support "top N" style displays — top 10 customers by sales, last 20 alerts, etc. To get a true "top N" semantic, also specify a [sort](ListGrid_2.md#attr-listgridinitialsort) so that the N records returned are the ones the user expects.
...
---
## Attr: ResultSet.generatedFieldSortRequiresCompleteCacheErrorMessage

### Description
Error message used when records cannot be ordered by a [generated field](../reference.md#kb-topic-fieldgeneration) because only part of the result set is cached locally.

This is the general explanation, used when the field's generator offers no more specific one. A generator that knows why it cannot sort - a field generated by AI, whose values cannot be produced for more than [DataBoundComponent.aiSortFieldMaxRecords](DataBoundComponent.md#attr-databoundcomponentaisortfieldmaxrecords) records - supplies its own message instead.
...
- [FieldGenerator.getNonSuccessfulRecordSortResultAtOutset()](#fieldgeneratorgetnonsuccessfulrecordsortresultatoutset)
