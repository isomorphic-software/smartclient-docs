...
To create an "icon-only" button — one showing just an [icon](#attr-statefulcanvasicon) with no title — set title to null. An explicit empty string ("") is not equivalent: it reserves space for an empty title and leaves the icon off-center, so use null rather than "".
