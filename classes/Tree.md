...
### Description
Creates a property index on this tree to accelerate [Tree.findAll](#method-treefindall) lookups from O(N) tree traversal to O(1) Map lookups. Once created, the index is automatically maintained as nodes are added or removed via [Tree.add](#method-treeadd) and [Tree.remove](#method-treeremove).

Indices support duplicate values: [Tree.findAll](#method-treefindall) returns all matching nodes. Index ordering matches insertion order, which may differ from tree traversal order.
...
### Description
Returns all nodes matching the specified field value. If a [property index](#method-treecreateindex) exists for the field, lookup is O(1); otherwise falls back to O(N) tree traversal.

### Parameters

| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| fieldName | [String](#type-string) | false | — | the name of the field to match |
| value | [Any](#type-any) | false | — | the value to match |

### Returns

`[Array of TreeNode](#type-array-of-treenode)` — array of matching nodes, or empty array if none found

### See Also

- [Tree.find](#method-treefind)
- [Tree.createIndex](#method-treecreateindex)
