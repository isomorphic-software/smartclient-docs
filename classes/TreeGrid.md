...
Except where explicitly overridden, [ListGrid](ListGrid_1.md#class-listgrid) methods, callbacks, and properties apply to TreeGrids as well. The [ListGrid](ListGrid_1.md#class-listgrid) defines some methods as taking/returning [ListGridField](../reference.md#object-listgridfield) and [ListGridRecord](../reference.md#object-listgridrecord). When using those methods in a TreeGrid, those types will be [TreeGridField](../reference.md#object-treegridfield) and [TreeNode](../reference.md#object-treenode), respectively.
...
### Description
The filename of the default icon for all closed folder nodes in this treeGrid. Use the node.icon property (null by default) to specify a custom image for an individual folder node. The same custom image will be used for both the open and closed folder images.
...
### Description
For [Multi-link trees](Tree.md#method-treeismultilinktree) only, the message displayed when the user attempts to drag two or more occurences of the same node into a parent.
...
### Description
If [TreeGrid.showConnectors](#attr-treegridshowconnectors) is true, this property determines whether we should show vertical continuation lines for each level of indenting within the tree. Setting to false will show only the hierarchy lines are only shown for the most indented path ("sparse" connectors).
...
### Description
When [TreeGrid.keepParentsOnFilter](#attr-treegridkeepparentsonfilter) is enabled for [fetchMode:"local"](../reference.md#type-fetchmode) ResultTrees, this property lists field names that will be sent to the server if they are present in the criteria.
...
May be overridden for [folderIcons](#attr-treegridfoldericon) by via [TreeGrid.folderIconPadding](#attr-treegridfoldericonpadding). May also be overridden for individual nodes by setting the [TreeGrid.iconPaddingProperty](#attr-treegridiconpaddingproperty) value on individual nodes
...
### Description
You can specify the initial set of data for a databound TreeGrid using this property. The value of this attribute should be a list of `parentId`\-linked [TreeNode](../reference.md#object-treenode)s in a format equivalent to that documented on [Tree.data](Tree.md#attr-treedata) or, for TreeGrids with [dataFetchMode](#attr-treegriddatafetchmode) set to ["paged"](../reference.md#type-fetchmode), on [ResultTree.data](ResultTree.md#attr-resulttreedata).
...
### Description
The filename of the default icon for all leaf nodes in this treeGrid. Use the node.icon property (null by default) to specify a custom image for an individual node.
...
### Description
The filename of the icon displayed for a folder node that will accept drag-and-drop data when the mouse is released.
...
`[URL](../reference.md#type-url)` — URL for the icon to show the node's open state
...
### Description
Overridden to return a [TreeGridViewState](../reference_2.md#type-treegridviewstate) object for the grid.

### Returns

`[TreeGridViewState](../reference_2.md#type-treegridviewstate)` — current view state for the grid.
...
- [TreeGridViewState](../reference_2.md#type-treegridviewstate)
...
### Description
Transfer a list of [TreeNode](../reference.md#object-treenode)s within this treeGrid or from from some other component (does not have to be a databound component) into this component. This method is only applicable to list-type components, such as [listGrid](ListGrid_1.md#class-listgrid), [treeGrid](#class-treegrid) or [tileGrid](TileGrid.md#class-tilegrid). Please see the paragraph below for special rules concerning [multi-link trees](Tree.md#method-treeismultilinktree).
...
`[URL](../reference.md#type-url)` — URL for the icon to show for this node
...
### Description
Overridden to take a [TreeGridViewState](../reference_2.md#type-treegridviewstate) object.

### Parameters

| Name | Type | Optional | Default | Description |
|------|------|----------|---------|-------------|
| viewState | [TreeGridViewState](../reference_2.md#type-treegridviewstate) | false | — | Object describing the desired view state for the grid |
...
`[URL](../reference.md#type-url)` — URL for the extra icon (null if none required)
...
### Description
Reset this set of open folders within this grid's data to match the [TreeGridOpenState](../reference.md#type-treegridopenstate) object passed in.  
...
| icon | [SCImgURL](../reference_2.md#type-scimgurl) | false | — | path to the resource |
...
During a drag-and-drop interaction, this method returns the set of node occurences being dragged out of the component, wrapped inside [NodeLocator](../reference_2.md#object-nodelocator)s. In the default implementation, this is the list of currently selected node occurences
...
`[Array of NodeLocator](#type-array-of-nodelocator)` — Array of [NodeLocator](../reference_2.md#object-nodelocator)s unambiguously identifying the node occurences that are currently selected
...
### Description
Returns a snapshot of the current open state of this grid's data as a [TreeGridOpenState](../reference.md#type-treegridopenstate) object.  
