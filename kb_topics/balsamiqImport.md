...
*   offer such functionality to your end users as a means of extending your application (_requires [Visual Builder OEM license](http://smartclient.com/product)_)
...
*   in Visual Builder: select the Screen menu option "Import from Balsamiq". You can provide the Balsamiq mockup file in various ways; providing a file will automatically load the screen created from the imported Balsamiq mockup into Visual Builder for further editing.

Then, in both cases, what you want to save for further work is the XML source generated by the tool, which is called [Component XML](componentXML.md#kb-topic-component-xml). In Visual Builder, you can copy and paste the XML from the Screen menu option "Show code for screen"; with the standalone tool you can press the "Show Source" button and copy and paste from there.
...
Balsamiq has a concept of [symbols](http://support.balsamiq.com/customer/portal/articles/110439), which are reusable elements that can be incorporated into multiple mockups. The Mockup Importer supports symbols, however, like linked mockups, symbols are only supported when the mockup file and symbol file are both present on disk and do not need to be uploaded. The symbols file should be in the same location relative to the mockup as Balsamiq expects it - _assets/symbols.bmml_ by default.
