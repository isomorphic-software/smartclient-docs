...
```
Generally, the version number wouldn't be hard-coded into the dynamic page, but would instead pick up the value of a variable, such that you can simply bump up the value in one configuration file and have all versioned URLs change dynamically.

Because SmartClient performs the assembly of image URLs dynamically, versioning of SmartClient skins must be done with a path component. You can do this by changing the base skinDir using [Page.setSkinDir](../classes/Page.md#classmethod-pagesetskindir) - e.g:

```
 isc.Page.setSkinDir('/version/5.6/isomorphic/skins/SmartClient/');
 
```
You can then either deploy the new skins under the versioned directory above or use a URL rewriting engine such as mod\_rewrite for Apache to map all such versions into a single deploy directory.
