...
Developers embedding third party text editing components into SmartClient widgets should typically set [Canvas.canSelectText](../classes/Canvas.md#attr-canvascanselecttext) to `true` on the SmartClient widget. This prevents the SmartClient event management system potentially interfering with selection and copy/paste behavior as the user interacts with the embedded editor (observed in some browsers, including some versions of Internet Explorer).
