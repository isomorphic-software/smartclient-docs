...
Previous versions applied criteria on the server for all fields **before** summarization. You can get that behavior back via [OperationBinding.filterBeforeGrouping](../classes/OperationBinding.md#attr-operationbindingfilterbeforegrouping) (or per-request via [DSRequest.filterBeforeGrouping](../classes/DSRequest.md#attr-dsrequestfilterbeforegrouping)). Note that in this case you need to turn off client-side filtering for aggregated fields, because client-side filtering cannot reproduce pre-summarization filtering: the client sees only the final computed aggregates. See [OperationBinding.filterBeforeGrouping](../classes/OperationBinding.md#attr-operationbindingfilterbeforegrouping) for more details.

**summaryRow:** [summaryRow](../classes/ListGrid_1.md#attr-listgridshowgridsummary) fetches automatically set `filterBeforeGrouping:true`, because the summaryRow uses the main grid's criteria and must filter rows before computing aggregates.

#### Explicit Post-Aggregation Criteria

In addition to the automatic criteria splitting described above, [DSRequest.afterCriteria](../classes/DSRequest.md#attr-dsrequestaftercriteria) can be used to explicitly specify criteria that should always be applied _after_ aggregation, regardless of the `filterBeforeGrouping` setting. This is conceptually equivalent to a SQL `HAVING` clause and is supported by the built-in SQL, Hibernate, and JPA connectors.

When both regular criteria and `afterCriteria` are supplied:

*   Regular criteria are split or applied according to the rules above (or placed entirely before aggregation if `filterBeforeGrouping` is set).
*   `afterCriteria` are always applied after aggregation.

For SQL DataSources, the [OperationBinding.afterWhereClause](#attr-operationbindingafterwhereclause) clause gives further control by allowing a bespoke SQL expression to replace or extend the generated post-aggregation clause via [SQL Templating](#kb-topic-customquerying).
...
However, [DSRequest.afterCriteria](../classes/DSRequest.md#attr-dsrequestaftercriteria) is fully supported for HB and JPA DataSources and provides explicit post-aggregation filtering when needed.
...
In clause-by-clause substitution there are two additional aggregation specific clauses: [groupClause](#attr-operationbindinggroupclause) providing "group by" part of aggregated query and [afterWhereClause](#attr-operationbindingafterwhereclause) providing "having" part of aggregated query (or outer "where" part if sub-select approach is used, see [OperationBinding.useHavingClause](../classes/OperationBinding.md#attr-operationbindingusehavingclause) for more details). The automatically generated `groupClause` and `afterWhereClause` clauses are also available as [$defaultGroupClause](#type-defaultqueryclause) and [$defaultAfterWhereClause](#type-defaultqueryclause) SQL templating variables.

Note that if [OperationBinding.filterBeforeGrouping](../classes/OperationBinding.md#attr-operationbindingfilterbeforegrouping) is set to `true`, aggregated fields referenced in regular criteria are filtered before aggregation occurs. In this case, `afterWhereClause` may still be generated, but only for criteria defined in [DSRequest.afterCriteria](../classes/DSRequest.md#attr-dsrequestaftercriteria).
