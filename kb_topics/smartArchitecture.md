...
## KB Topic: SmartClient Architecture

### Description
SmartClient can add interactivity and performance benefits to any web application with a variety of integration approaches. This topic discusses the optimal architecture for a SmartClient application, which can be adopted in whole or in part.
...
A dynamic, data-driven UI can be expressed completely in _cacheable_ JavaScript. This is in contrast to any architecture based on server-side HTML generation, where static parts of the presentation are mixed in with dynamic data, preventing cacheability so that bandwidth and server time are wasted repeatedly delivering the same static presentation data.  
Even generated JavaScript is cacheable. For example, a SmartClient View expressed in XML and contained within a JSP is still a separately cacheable resource when loaded via a `<SCRIPT SRC>` tag and advertised as a cacheable resource via HTTP headers, because it is ultimately delivered to the browser as simple JavaScript. Hence standard [internationalization](i18n.md#kb-topic-internationalization-and-localization) techniques such as using JSTL tags in a JSP remain applicable.  
The SmartClient Architecture even allows you to capture all the gradations of cacheability from completely static (changes once per application rollout) to completely dynamic (timestamp). In the example above, the user name wouldn't actually change for the lifetime of the page, so could be loaded once only.  
 

**Performance: Minimal Server State**

Any architecture that relies on component descriptions being generated by the server must track a great deal of state, which, in the SmartClient Architecture, is either completely eliminated or greatly reduced.  
 
...
Using the SmartClient architecture allows you to build a complete, working application that can run without a server, based on sample data. In the example above, it would be straightforward to create a testing mode that returned a faked RPC response consisting of simply `{ userName : "Bob" }`.  
...
For more info on creating applications that support client-only testing, see [Client Only DataSources](clientOnlyDataSources.md#kb-topic-client-only-datasources).  
 
...
Many views will be shown to the user repeatedly, for example, the user may repeatedly switch back and forth between two panes of a TabSet. In that usage it makes sense to make a distinction between _creating_ a view and _showing_ an existing view. When showing an existing view, the same components and/or data may be able to be reused.  
In the following variant on the original example, we only create the Window object and do the RPC to retrieve the user name the first time `showNewUserWizard()` is called. Subsequently we reuse the existing window, and we assume the user name has not changed, so we need not do the RPC again. (_Note: "New User" button omitted for brevity from here on_)
```
    function showNewUserWizard() {
        if (!window.myWindow) {
            Window.create({
...
```
**Batching Operations**  
A view may incorporate multiple components, each of which requires data. In the following example, a DataBound ListGrid has been incorporated into the wizard, and we'd like to fetch the user's name and the beginning dataset for the grid in the same batch. We use [RPCManager.startQueue](../classes/RPCManager.md#classmethod-rpcmanagerstartqueue) to do so.
```
    function showNewUserWizard() {
        if (!window.myWindow) {
            Window.create({
