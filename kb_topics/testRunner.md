...
These DataSources are present in the `smartclientSDK/tools/selenium/` directory of your SDK. If you choose to move them elsewhere, simply update the DataSources location (configured by `project.datasources` in [server.properties](../reference.md#kb-topic-serverproperties-file)).
...
If needed, the IDs of these DataSources can be configured via the [server.properties](../reference.md#kb-topic-serverproperties-file) settings `autotest.batchRunDS` and `autotest.testResultDS`.
...
TestRunner requires several resources to be in expected default locations from the current directory unless you provide overrides via the command line or [server.properties](../reference.md#kb-topic-serverproperties-file). Some of the required resources are:
...
| Command-line Option | Java Setter | [server.properties](../reference.md#kb-topic-serverproperties-file) Name | Behavior |
|---|---|---|---|
