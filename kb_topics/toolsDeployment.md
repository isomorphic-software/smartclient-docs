...
To deploy the tools simply copy the `tools` directory into your deployment. There are no additional settings to configure.
...
#### Developer Console
Unlike the other tools, the Developer Console is always safe to deploy to production environments. On its own, it only exposes the kind of information that an end user is already able to get using browser tools such as Firebug. By default then, it is always available at \[webroot\]/isomorphic/system/helpers/Log.html.
