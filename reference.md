...
- [Class](classes/Class.md)
  - [BaseWidget](classes/BaseWidget.md)
    - [Canvas](classes/Canvas.md)
      - [Layout](classes/Layout.md)
        - [VLayout](#class-vlayout)
          ...
          - [Card](classes/Card.md)
            - [BorderedCard](#class-borderedcard)
              - [BorderedPanel](#class-borderedpanel)
            - [ElevatedCard](#class-elevatedcard)
      ...
      - [CanvasGrid](classes/CanvasGrid.md)
  ...
  - [FormItem](classes/FormItem.md)
    ...
    - [CanvasItem](classes/CanvasItem.md)
      ...
      - [FileUploadItem](classes/FileUploadItem.md)
...
- [Batch Widget Creation](kb_topics/batchCreation.md)
...
- [Color Overview](kb_topics/colorOverview.md)
...
- [Full Text Search](kb_topics/fullTextSearch.md)
...
- [Process Input and Output Schema](kb_topics/processIO.md)
...
- [Adding Custom Workflow Tasks](kb_topics/reifyAddWorkflowTask.md)
...
*Inherits from:* [Label](classes/Label.md#class-label)

### Description
A [Label](classes/Label.md#class-label) subclass for headings, styled by the "headerItem" [CSS style-series](classes/StatefulCanvas.md#attr-statefulcanvasbasestyle) - the same series [HeaderItem](classes/HeaderItem.md#class-headeritem) uses inside a [DynamicForm](classes/DynamicForm.md#class-dynamicform), so a heading placed above a form matches the headings within it.

Defaults to `width:"100%"` and `height:50`, so a `HeaderLabel` added to a [VLayout](#class-vlayout) spans the layout and reserves the same band for the heading whatever text it carries. Both are ordinary defaults; override either as you would on any [Canvas](classes/Canvas.md#class-canvas).
...
This task is primarily used in the workflow editor to render a "no-op" task or as an explicit visual marker for the end of workflow. Works inside [server-side processes](classes/OperationBinding.md#attr-operationbindingprocess).
...
### Description
[Card](classes/Card.md#class-card) with prominent border. Uses `styleName: "borderedPanel"`.
...
*Inherits from:* [BorderedCard](#class-borderedcard)

### Description
Alias for [BorderedCard](#class-borderedcard). Deprecated — use `BorderedCard` instead.
...
### Description
[Card](classes/Card.md#class-card) with more pronounced shadow. Uses `styleName: "elevatedPanel"`.
...
*Inherits from:* [ElevatedCard](#class-elevatedcard)

### Description
Alias for [ElevatedCard](#class-elevatedcard). Deprecated — use `ElevatedCard` instead.
...
### Description
Alias for [Card](classes/Card.md#class-card). Deprecated — use `Card` instead.
...
### Description
[Card](classes/Card.md#class-card) with border only, no shadow. Uses `styleName: "flatPanel"`.
...
*Inherits from:* [FlatCard](#class-flatcard)

### Description
Alias for [FlatCard](#class-flatcard). Deprecated — use `FlatCard` instead.
...
- [AI.getDataSourceSummary](classes/AI.md#classmethod-aigetdatasourcesummary)
- [AI.getDataSourceSummaries](classes/AI.md#classmethod-aigetdatasourcesummaries)
...
For further information, setup instructions and example configuration, see our [public wiki](https://isomorphic.atlassian.net/wiki/spaces/Main/pages/524533/Google+App+Engine)
...
| "preferLocal" | Combine both sets of records, but report the search record from [adminSavedSearches](classes/DataBoundComponent.md#attr-databoundcomponentadminsavedsearches) if two records have the same search name. |
| "localOnly" | Only report the records from [adminSavedSearches](classes/DataBoundComponent.md#attr-databoundcomponentadminsavedsearches). |
...
### Description
Controls when a databound component that has no [ResultSet](classes/ResultSet.md#class-resultset) to keep it current — a [DynamicForm](classes/DynamicForm.md#class-dynamicform), [ValuesManager](classes/ValuesManager.md#class-valuesmanager) or [DetailViewer](classes/DetailViewer.md#class-detailviewer) — picks up changes to the record it is showing.

Two distinct cases are covered. First, a save may be altered by the server: sequence values, defaults, computed fields and server-side business logic can all mean the stored record differs from what was submitted, and the component should show what was actually stored. Second, the same record may be edited somewhere else — another form, a grid on a different screen, or a real-time update applied via [DataSource.updateCaches](classes/DataSource_1.md#method-datasourceupdatecaches) — leaving the component showing superseded values that a user has no reason to think are stale, and may go on to commit.

### Values

| Value | Description |
|-------|-------------|
| "always" | Pick up changes from any successful "update" against the DataSource, whichever component or [DataSource.updateCaches](classes/DataSource_1.md#method-datasourceupdatecaches) call made them, as well as changes the server made to this component's own save. This is the default, and is the only setting that covers both cases described above. |
| "whenSaving" | Pick up only the changes the server made to this component's own save. Updates originating elsewhere are ignored, so the component goes on showing what it last saved or was last given. |
| "never" | Never update from a save response at all. The component shows only the values the application puts into it.

An update never discards an edit the user has made and not yet saved. A field the user has changed keeps the value they typed, and goes on reporting as changed via [DynamicForm.valuesHaveChanged](classes/DynamicForm.md#method-dynamicformvalueshavechanged), so the [oldValues](kb_topics/concurrentEdits.md#kb-topic-handling-concurrent-edits-in-smartclient-datasources) sent with their eventual save still describe the record they started from. The exception is a field where the update happens to carry the same value the user typed: that edit has already been stored by whoever made the update, so it stops counting as an unsaved change. |

### See Also

- [DynamicForm.cacheSync](classes/DynamicForm.md#attr-dynamicformcachesync)
- [ValuesManager.cacheSync](classes/ValuesManager.md#attr-valuesmanagercachesync)
- [DetailViewer.cacheSync](classes/DetailViewer.md#attr-detailviewercachesync)
...
| Value | Description |
|-------|-------------|
| "omit" | Event properties are omitted entirely. |
| "actionBinding" | Events are typed as Action binding objects (target + action name + optional mapping). |
| "declarative" | Events are typed as full Process definitions (no plain-string mode). |
...
### Description
Position of a [ListGrid.recordDrop](classes/ListGrid_3.md#method-listgridrecorddrop) operation with respect to the target record.
...
#### Standalone Evaluator Support
For server-side Java code that needs to evaluate AdvancedCriteria containing `fieldQuery` or `valueQuery` outside of the normal DSRequest flow, the `com.isomorphic.criteria.Evaluator` class provides direct support. The `filterRecords()` method accepts a list of records and automatically resolves any subqueries before filtering. You can either provide a `SubqueryResolver` via the constructor, or use the convenience overload that accepts a parent `DataSource` and auto-creates a `DSRequestSubqueryResolver`. For scenarios where data is not in a database (e.g., in-memory caches, file-based data), `InMemorySubqueryResolver` can resolve subqueries against in-memory collections. See the Evaluator Javadoc for details.

#### Composite Foreign Key Support
Subqueries support composite foreign keys, where multiple fields form the key. When a subquery's join to the parent DataSource involves a composite FK, all participating key fields are used in the join constraint automatically.
...
`fieldQuery` and `valueQuery` are supported for purely client-side DataSources: `clientOnly` DataSources, [MockDataSource](classes/MockDataSource.md#class-mockdatasource)s, and any DataSource whose [ResultSet](classes/ResultSet.md#class-resultset) cache is complete, meaning all rows have been fetched from the server.

When filtering client-side data with AdvancedCriteria that contain subquery criteria, subqueries are resolved against the target DataSource's local ResultSet cache, and the resolved values are used for filtering. The `queryFK` join constraints, `summaryFunctions`, `groupBy`, and `queryOutput` properties are all respected.
...
- [AsyncDataBoundOperationContext](reference_2.md#object-asyncdataboundoperationcontext)
...
### Description
An object that contains information needed to evaluate an [operator](reference_2.md#object-operator) on a record.
...
### Description
Shape of the infrastructure-failure record delivered to [Process.failed](classes/Process.md#method-processfailed) and (for [CoTProcess](classes/CoTProcess.md#class-cotprocess)) via the async operation's failure result. Recoverable, schema-conformant errors do NOT use this type; they live inside the Process's successful output (see [processIO](kb_topics/processIO.md#kb-topic-process-input-and-output-schema)).
...
---
## Object: ReifyDSSettings

### Description
Optional settings object passed to [Reify.getProjectsDS](classes/Reify.md#classmethod-reifygetprojectsds), [Reify.getScreensDS](classes/Reify.md#classmethod-reifygetscreensds), and [Reify.getDataSourcesDS](classes/Reify.md#classmethod-reifygetdatasourcesds).
...
- [EventStream.transformSelenese](classes/EventStream.md#method-eventstreamtransformselenese)
...
### Description
One entry of [addedSteps](classes/WorkflowBuilderCoTResult.md#attr-workflowbuildercotresultaddedsteps) - a single Workflow step added by a [WorkflowBuilderProcess](#class-workflowbuilderprocess) run.
...
### Description
The shape of the resolved output of a [WorkflowBuilderProcess](#class-workflowbuilderprocess) run, available via [AsyncOperationResult.output](#asyncoperationresultoutput) once [CoTProcess.asyncStart](classes/CoTProcess.md#method-cotprocessasyncstart) completes.
