...
*   `criteria.*` — the value the client supplied in its request criteria for the named field. Example: `criteria.startDate notNull` rejects if the client omitted `startDate` from its criteria. In `operationBinding.criteria`, `valuePath="criteria.qualityRating"` uses the client's `qualityRating` criterion value to constrain another column (e.g. requiring `peerRating` to meet the same threshold the user chose for `qualityRating`).
*   `submitted.*` — the value the client included in the submitted record (`dsRequest.getValues()`). Example: `submitted.quantity lessThan 1000` caps order size.

These prefixes are server-only. The client defers criteria containing these prefixes to the server for evaluation.

All prefixes support a `$` disambiguation form (`$criteria.*`, `$submitted.*`, `$auth.*`, `$context.*`) to force server-side prefix interpretation when a DataSource with the same name exists. For example, if a DataSource named "submitted" exists, `$submitted.quantity` references the client-submitted value, while `submitted.quantity` without the `$` would be treated as a relational reference to the "submitted" DataSource.
