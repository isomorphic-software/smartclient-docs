...
### Description
Constants for use with [Canvas.getAllCanvases](classes/Canvas.md#classmethod-canvasgetallcanvases) to specify which canvases to return.

### Values

| Value | Description |
|-------|-------------|
| "visible" | Return only canvases that are drawn, visible to the end user, and non-generated (developer-created). This is the default. Visibility checking includes parental visibility, so a canvas whose parent is hidden will not be returned. |
| "visibleIncludeGenerated" | Return only canvases that are drawn and visible to the end user, including generated canvases (autoChildren, etc.). |
| "drawn" | Return only canvases that are drawn (regardless of visibility or generated status). |
| "drawnHidden" | Return only canvases that are drawn but hidden (either directly hidden or hidden due to a hidden parent). |
| "undrawn" | Return only canvases that are not yet drawn. |
| "generated" | Return only generated canvases (autoChildren and other framework- generated canvases) that are drawn and visible. |
| "all" | Return all canvases regardless of drawn/visibility/generated state. |
| "allGenerated" | Return all generated canvases regardless of drawn/visibility state. |
| "allNonGenerated" | Return all non-generated canvases regardless of drawn/visibility state. |
...
---
## Type: DataPointSizeScalingMode

### Description
The scaling mode used to map [FacetChart.pointSizeMetric](classes/FacetChart.md#attr-facetchartpointsizemetric) values to data point sizes.
...
- [FacetChart.dataPointSizeScaling](classes/FacetChart.md#attr-facetchartdatapointsizescaling)
...
| Value | Description |
|-------|-------------|
| "none" | No descriptions are output. |
...
| "objects" | Server-side only. Export data as a List of Maps with field-name keys and fully-processed display values (dates formatted, valueMaps applied). No file or stream is produced; retrieve the result via `dsResponse.getExportObject()` in server-side Java code. |
...
*   `auth.userId` — the authenticated user ID. Available on both server and client.
*   `auth.currentUser.userId` — equivalent to `auth.userId`. On the client, the full `auth.currentUser` object (set via [Authentication.setCurrentUser](classes/Authentication.md#classmethod-authenticationsetcurrentuser)) is also available, providing access to any user properties (e.g. `auth.currentUser.firstName`). On the server, `auth.currentUser` contains only `userId`.
*   `auth.roles` — the list of roles for the authenticated user. Available on both server and client.
...
#### Request criteria and submitted values
The following prefixes provide server-side access to the client's request data within [OperationBinding.requiresWhen](classes/OperationBinding.md#attr-operationbindingrequireswhen) and [OperationBinding.criteria](classes/OperationBinding.md#attr-operationbindingcriteria):

*   `criteria.*` — the value the client supplied in its request criteria for the named field. In `requiresWhen`, this acts as a gate on the request criteria — for example, `criteria.startDate notNull` rejects if the client omitted `startDate`. In `operationBinding.criteria`, `criteria.*` references are added to the client-supplied criteria, tightening the result set with server-enforced constraints.
*   `values.*` — the value the client included in the submitted record (`dsRequest.getValues()`). Example: `values.quantity lessThan 1000` caps order size.
*   `storedRecord.*` — the pre-modification database record (update/remove only). Example: `storedRecord.status notEqual "locked"` prevents updates to locked records. The stored record is immune to client manipulation — unlike `values.*`, a malicious client cannot influence its contents.
*   `mergedRecord.*` — the stored record overlaid with client-submitted values. Fields the client submitted override the stored value; fields the client did not submit retain their stored value. Example: `mergedRecord.quantity lessOrEqual 50` ensures the _resulting_ quantity after the update does not exceed 50, regardless of whether the client submitted a new quantity or left the stored one unchanged.

`storedRecord.*` and `mergedRecord.*` are meaningful for **update** and **remove** operations only. On **add**, `mergedRecord.*` falls back to `values.*` (no stored record to merge with); `storedRecord.*` forces the criterion to fail (there is no prior record). On **fetch**, both force the criterion to fail.

When no prefix is used, the default evaluation record depends on the operation type: for update/remove, the stored record; for add, the submitted values.

**Security note:** `storedRecord.*` should be used for security gates that lock records based on their current state (e.g. preventing updates to “Approved” records). Using `mergedRecord.*` for such a gate is a security vulnerability — a malicious client could submit `status:“Draft”` to bypass the lock. Use `mergedRecord.*` only for rules that validate the _resulting_ state (e.g. “the resulting quantity must not exceed 50 when status is Approved”).

**Wrong** — security gate uses `mergedRecord.*`:

```
 <!-- WRONG: client can submit status="Draft" to
      bypass this gate -->
 <requiresWhen fieldName="mergedRecord.status"
               operator="notEqual"
               value="Approved"/>
 
```
**Right** — security gate uses `storedRecord.*`:
```
 <!-- RIGHT: gate checks the actual DB value, which
      the client cannot influence -->
 <requiresWhen fieldName="storedRecord.status"
               operator="notEqual"
               value="Approved"/>
 
```
For rules about the _resulting_ state, use `mergedRecord.*`:
```
 <!-- Resulting-state rule: high-quantity orders
      cannot be set to Approved -->
 <requiresWhen operator="or">
   <criteria fieldName="mergedRecord.status"
             operator="notEqual"
             value="Approved"/>
   <criteria fieldName="mergedRecord.quantity"
             operator="lessOrEqual" value="50"/>
 </requiresWhen>
 
```

All prefixes support a `$` disambiguation form (`$criteria.*`, `$values.*`, `$storedRecord.*`, `$mergedRecord.*`, `$auth.*`, `$context.*`) to force prefix interpretation when a DataSource with the same name exists. For example, if a DataSource named "values" exists, `$values.quantity` references the client-submitted value, while `values.quantity` without the `$` would be treated as a relational reference to the “values” DataSource.
...
#### requiresWhen vs operationBinding.criteria
Both [OperationBinding.requiresWhen](classes/OperationBinding.md#attr-operationbindingrequireswhen) and [OperationBinding.criteria](classes/OperationBinding.md#attr-operationbindingcriteria) use ServerDynamicCriteria, but they solve different problems.

|  | requiresWhen | operationBinding.criteria |
|---|---|---|
| Purpose | Security gate: allow or reject the entire operation before it executes | Invisible constraint: additional criteria merged into the operation's request so restricted data is silently excluded |
| When it runs | Before the operation — evaluated during security checks, before any data access | Becomes part of the operation — merged into the DSRequest criteria and processed by the server type's native filtering |
| Result when criteriadon't match | Operation is rejected with an error response (status < 0). The client knows the operation was denied. | No error — the query returns fewer rows (or zero) for fetches, or affects fewer records (or none) for updates/removes. The constraint is invisible to the client. |
| Data access | Works with any server type. For pure SQL, evaluates in a single database query; for non-SQL or cross-database, uses individual lookups per relational reference. | Works with any server type. Criteria are merged into the DSRequest and handled by the server type's own query mechanism — no additional queries. |

Use `requiresWhen` when the client should know the operation was denied (security, business rules). Use `operationBinding.criteria` when the constraint should be invisible (row-level security, data partitioning, tenant isolation).
...
_Server:_ implemented as a native SQL aggregate expression by the Oracle (LISTAGG), PostgreSQL (string\_agg), and MySQL (GROUP\_CONCAT) database drivers. Other SQL drivers perform an additional query to fetch the values for concatenation. Also supported for generic dataSources that do not have inherent support for aggregation — see the description of this in the [allowAggregation](classes/DataSource_1.md#attr-datasourceallowaggregation) documentation. Not supported by JPA or Hibernate dataSources. See also [joinPrefix](classes/DataSourceField.md#attr-datasourcefieldjoinprefix), [joinString](classes/DataSourceField.md#attr-datasourcefieldjoinstring) and [joinSuffix](classes/DataSourceField.md#attr-datasourcefieldjoinsuffix) related datasource field attributes. |
| concatDistinct | _Client:_ like "concat", but removes duplicate values before concatenation.  
_Server:_ natively supported on Oracle 19c+ (LISTAGG DISTINCT), PostgreSQL 9.0+ (string\_agg DISTINCT), and MySQL (GROUP\_CONCAT DISTINCT). On other SQL databases, deduplication is applied programmatically after fetching values. Also supported client-side for clientOnly and cached dataSources. See also [joinPrefix](classes/DataSourceField.md#attr-datasourcefieldjoinprefix), [joinString](classes/DataSourceField.md#attr-datasourcefieldjoinstring) and [joinSuffix](classes/DataSourceField.md#attr-datasourcefieldjoinsuffix) related datasource field attributes. |
...
`TaskInputExpressions` are designed to be visual-tool-friendly so that it's easy to visually wire together processes in the [Workflow Editor](kb_topics/reifyAddWorkflowTask.md#kb-topic-adding-custom-workflow-tasks).
...
### Description
Optional settings for [Reify.showOverlay](classes/Reify.md#classmethod-reifyshowoverlay).
